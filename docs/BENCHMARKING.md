...
Use the client console to record frame counters, component CPU zones, and native Vulkan/WebGPU GPU timestamps in memory and save them as JSON. Supported native backends additionally split GPU work into the zones the game registers (the world, the interface, and the components drawn inside them):
