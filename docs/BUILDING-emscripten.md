...
-	Without a browser, Node from the emsdk runs the headless client (`-DHEADLESS_CLIENT=ON`) against a native server over WebSockets: in the build directory, `node -r <repo>/scripts/emscripten/node-xhr-stub.js DDNet.js "connect ddnet+ws://127.0.0.1:8303"`.
	...
	The integration test `browser_client_can_connect` does the same: `scripts/integration_test.py --test-websockets --emscripten-client <build>/DDNet.js <native server build dir> browser_client_can_connect`, with a native server built with `-DWEBSOCKETS=ON`; the CI's Emscripten job runs it.
