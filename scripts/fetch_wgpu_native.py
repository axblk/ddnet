...
# Android is cross-compiled, so its packages are asked for by name, one ABI at
# a time, and go into a directory of their own that WGPU_NATIVE_ROOT points at.
CROSS_PACKAGES = {
	"android-arm64-v8a": (
...
def main() -> None:
	...
	if arguments.target is not None:
		...
		# Windows' Python says AMD64 where MSYS2's says x86_64.
		machine = "x86_64" if platform.machine() == "AMD64" else platform.machine()
		package = PACKAGES.get((platform.system(), machine))
		if package is None:
