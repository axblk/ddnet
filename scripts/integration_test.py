...
class TestRunner:
	def __init__(self, ddnet, ddnet_server, ddnet_mastersrv, teeworlds_client, repo_dir, test_dir, show_full_output, test_websockets, test_quic, test_baseline, quic_certificate, quic_private_key, quic_wrong_certificate, quic_wrong_private_key, valgrind_memcheck, keep_tmpdirs, timeout_multiplier):
		self.ddnet = ddnet
...
def test(test=None, *, requires_mastersrv=False, requires_websockets=False, requires_teeworlds_client=False, requires_quic=False, requires_baseline=False, requires_linux=False, timeout=60):
	def apply(test):
