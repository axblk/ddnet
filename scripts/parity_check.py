	...
	with open(path, encoding="utf-8") as file:
		for number, line in enumerate(file, 1):
			...
			for scope in fields.split(","):
				scope = scope.strip()
				written = scope
				# A waiver that only applies from some tick on says so, and the
				# check holds it to it: `fire_grenade:pos@81` covers tick 81 and
				# later and nothing before, so a reason that names a tick is a
				# claim the run has to bear out rather than prose.
				scope, _, first_tick = scope.partition("@")
				if first_tick:
					if not first_tick.isdigit():
						sys.exit(f"{path}:{number}: {written!r} must end in @<tick>, a whole number")
					first_tick = int(first_tick)
				else:
					first_tick = 0
				if ":" not in scope:
					sys.exit(f"{path}:{number}: {written!r} must be scenario:field, or *:field for every scenario")
				if scope in entries:
					sys.exit(f"{path}:{number}: {scope} is already waived by {entries[scope][0]}")
				entries[scope] = (name, reason, first_tick)
	return entries


def waiver(ledger, scenario, field, tick):
	"""The scope that waives this field at this tick, or None.
	...
	supposed to prevent rather than cause. A scope written with `@<tick>` also
	stops covering everything before that tick. The scope is returned rather
	than the entry, because staleness is per scope: an entry covering four of
	them is not alive just because one still reproduces."""
	for scope in (f"{scenario}:{field}", f"*:{field}"):
		entry = ledger.get(scope)
		if entry is not None:
			return scope if int(tick) >= entry[2] else None
	return None
...
def main():
	...
	for key, server_fields in server.items():
		...
		for field, value in server_fields.items():
			if client_fields.get(field) == value:
				continue
			scope = waiver(ledger, key[1], field, key[2])
	...
	for scope, (name, _reason, _first_tick) in sorted(ledger.items()):
	...
	print(f"parity: {len(server)} lines match, {len(set(entry[0] for entry in ledger.values()))} known divergence(s)")
	return 0
