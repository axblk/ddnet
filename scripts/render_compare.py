...
The client is started once per backend, the demo is moved to a fixed time and
...
The client runs with cl_save_settings 0, so the saved config is left alone.
"""
...
# Entities at half opacity over the design, so a frame that is not cleared
# shows the one before bleeding through. Settings that change the picture are
# pinned here.
SCENE = ["cl_save_settings 0", "cl_overlay_entities 50", "gfx_screen_width 1024", "gfx_screen_height 768"]
