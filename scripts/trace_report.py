...
trace, so a full trace is a report of tens of kilobytes rather than tens of
...
# Zones the game times around the others. Adding them to the zones inside them
...
def classify(cost):
	"""Split the frames into hitches, the tail and the middle they are compared against."""
	...
	base = [i for i, c in enumerate(cost) if low <= c <= high] or list(range(len(cost)))
	return hitches, tail, base, hitch_limit


def summarise(series, tail, base):
	return {
...
def analyse_components(trace, own_times, num_frames, tail, base):
	names = trace["names"]
	zone_names = trace["gpu_zone_names"]
	name_zones = trace.get("name_zones", [])
	components = {}
	for index, name in enumerate(names):
		zone = name_zones[index] if index < len(name_zones) else -1
		series = [own_times[f].get(index, 0) for f in range(num_frames)]
		if any(series):
			components[name] = {"name": name, "zone": zone_names[zone] if 0 <= zone < len(zone_names) else None, **summarise(series, tail, base)}
	return components


def analyse_zones(trace, tail, base):
	zones = {}
	for index, zone_name in enumerate(trace["gpu_zone_names"]):
		series = [f["gpu_zones_ns"][index] if index < len(f["gpu_zones_ns"]) else 0 for f in trace["frames"]]
		if any(series):
			zones[zone_name] = {"name": zone_name, **summarise(series, tail, base)}
	return zones


def analyse_counters(frames, cost, tail, base):
	counters = []
	for key, title in COUNTERS:
		...
		try:
			...
			continue
		counters.append({"title": title, "r": correlation, "base": mean_of(base, series), "tail": mean_of(tail, series)})
	counters.sort(key=lambda c: -abs(c["r"]))
	return counters


def analyse_strip(frames, names, own_times, cost):
...
def analyse_worst(frames, names, own_times, cost, hitches):
	...
	listed = sorted(hitches or range(len(frames)), key=lambda i: -cost[i])
	worst = []
	for index in listed[:WORST_FRAMES]:
		...
		worst.append({
			...
			"parts": parts[:8],
			# What no scope explains, rather than blaming the largest component.
...
def analyse(trace, label):
	...
	# does is paid for in the frametime of the frame after it. Blaming a component
	# for a slow frame has to use this shifted cost. The last frame has no
	# successor and stands in with its own render time.
	cost = frametimes[1:] + [frames[-1]["render_wall_ns"]]
	hitches, tail, base, hitch_limit = classify(cost)
	...
	return {
		...
		"components": analyse_components(trace, own_times, len(frames), tail, base),
		"zones": analyse_zones(trace, tail, base) if gpu_supported else {},
		"counters": analyse_counters(frames, cost, tail, base),
		"strip": analyse_strip(frames, names, own_times, cost),
		"worst": analyse_worst(frames, names, own_times, cost, hitches),
...
def synthetic(stall_at=None):
	...
	return {"format": FORMAT, "version": VERSION, "dropped_frames": 0, "dropped_events": 0, "names": names, "name_zones": [-1, 0, 1], "gpu_zone_names": ["world", "hud"], "frames": frames, "events": events}
