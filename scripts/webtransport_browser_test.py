...
"""
		...
		if uses_webtransport:
			...
			server_arguments.extend([
				...
				"sv_quic 0",
				"sv_webtransport 1",
				f"sv_tls_cert {quic_certificate}",
				f"sv_tls_key {quic_private_key}",
			])
			# A name for the certificate makes the server rely on Web PKI.
			if args.transport == "webpki":
				server_arguments.append("sv_register_hostname 127.0.0.1")
