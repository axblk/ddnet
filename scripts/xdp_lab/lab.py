...
def a_quic_client_plays_through_the_filter(test_env):
	...
	xdp_filter, server = start(test_env, server_args=["sv_ipv4only 1", "sv_quic 1", *certificate.server_args()])
	client = test_env.client(["cl_connect_protocol 1", f"cl_quic_cert {certificate.sha256}", "cl_quic_server_name localhost"])
	client.wait_for_startup()
	connect(test_env, client)
	play(client, server)
	if xdp_filter is not None and total(lab.stats(xdp_filter), "drop") != 0:
...
def build_runner(args, lab):
	...
	if args.test_quic:
		quic_cli = os.path.join(args.builddir, "quic_cli")
		quic_certificates = (it.QuicCertificate.generate(quic_cli, args.builddir, "xdp-lab"), it.QuicCertificate.generate(quic_cli, args.builddir, "xdp-lab-other"))
	return LabRunner(
		lab,
