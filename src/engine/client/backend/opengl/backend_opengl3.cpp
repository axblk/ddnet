...
// WebGL2 fixes a buffer's type at its first bind
// (https://www.khronos.org/registry/webgl/specs/latest/2.0/#5.1), so an index
// buffer has to be created as one.
...
// The element array binding belongs to the bound vertex array object, so
// uploads go through none.
...
bool CGLSL::LoadShader(CGLSLCompiler *pCompiler, const char *pName, int Type)
{
	...
	if(BackendType == BACKEND_TYPE_OPENGL_ES)
	{
		...
		{
			// Uniforms both stages declare must agree in precision, and ints
			// default to highp in the vertex stage but mediump here.
			vLines.emplace_back("precision highp int; \r\n");
	...
	// Fed line by line for the ES rewriting, dialect header first.
...
void CCommandProcessorFragment_OpenGL3_3::LinkProgram(CGLSLTWProgram *pProgram, CGLSLCompiler &Compiler, const char *pVertexShader, const char *pFragmentShader, const std::vector<std::pair<const char *, std::string>> &vDefines)
{
	for(const auto &[pName, Value] : vDefines)
		Compiler.AddDefine(pName, Value.c_str());
	CGLSL VertexShader;
	CGLSL FragmentShader;
	VertexShader.LoadShader(&Compiler, pVertexShader, GL_VERTEX_SHADER);
	FragmentShader.LoadShader(&Compiler, pFragmentShader, GL_FRAGMENT_SHADER);
...
void CCommandProcessorFragment_OpenGL3_3::InitPrimExProgram(CGLSLPrimitiveExProgram *pProgram, CGLSLCompiler &Compiler, bool Textured)
{
	LinkProgram(pProgram, Compiler, "primex.vert", "primex.frag", Textured ? std::vector<std::pair<const char *, std::string>>{{"TW_TEXTURED", ""}} : std::vector<std::pair<const char *, std::string>>{});
...
bool CCommandProcessorFragment_OpenGL3_3::Cmd_Init(const SCommand_Init *pCommand)
{
	...
	LinkProgram(m_pPrimitiveProgramTextured, ShaderCompiler, "prim.vert", "prim.frag", {{"TW_TEXTURED", ""}});
	LinkProgram(m_pBlurProgram, ShaderCompiler, "prim.vert", "blur.frag");
	LinkProgram(m_pPlanarYuvProgram, ShaderCompiler, "prim.vert", "planar_yuv.frag");
	...
	const std::pair<CGLSLQuadProgram *, std::vector<std::pair<const char *, std::string>>> aQuadPrograms[] = {
		{m_pQuadProgram, {{"TW_MAX_QUADS", MaxQuads}}},
		{m_pQuadProgramTextured, {{"TW_QUAD_TEXTURED", ""}, {"TW_MAX_QUADS", MaxQuads}}},
		{m_pQuadProgramGrouped, {{"TW_QUAD_GROUPED", ""}}},
		{m_pQuadProgramTexturedGrouped, {{"TW_QUAD_TEXTURED", ""}, {"TW_QUAD_GROUPED", ""}}},
	};
	for(const auto &[pProgram, vDefines] : aQuadPrograms)
	{
		LinkProgram(pProgram, ShaderCompiler, "quad.vert", "quad.frag", vDefines);
		pProgram->m_LocColors = pProgram->GetUniformLoc("gVertColors");
		pProgram->m_LocRotations = pProgram->GetUniformLoc("gRotations");
		pProgram->m_LocOffsets = pProgram->GetUniformLoc("gOffsets");
		pProgram->m_LocQuadOffset = pProgram->GetUniformLoc("gQuadOffset");
	}

	LinkProgram(m_pTextProgram, ShaderCompiler, "text.vert", "text.frag");
	...
	LinkProgram(m_pSpriteProgramMultiple, ShaderCompiler, "spritemulti.vert", "spritemulti.frag");
...
int CCommandProcessorFragment_OpenGL3_3::ToGLInternalFormat(IGraphics::ETextureFormat Format)
{
	// OpenGL ES 3.0 and WebGL 2 need sized internal formats for RED and RG.
	switch(Format)
	{
	case IGraphics::ETextureFormat::RGBA8_UNORM: return GL_RGBA;
	case IGraphics::ETextureFormat::RG8_UNORM: return GL_RG8;
	}
	dbg_assert(false, "Unknown texture format");
	dbg_break();
}
...
void CCommandProcessorFragment_OpenGL3_3::TextureCreate(int Slot, const IGraphics::CTextureDesc &Desc, uint8_t *pTexData)
{
	...
	const int GLStoreFormat = ToGLInternalFormat(Desc.m_Format);
...
#if defined(CONF_PLATFORM_EMSCRIPTEN)
	// WebGL 2 cannot map buffers for reading, so the pixels are read
	// synchronously. No flip, as below.
	if(!pCommand->m_pResult->m_Image.TryReuse(Texture.m_Width, Texture.m_Height, CImageInfo::FORMAT_RGBA))
		return;
	{
		...
		glReadPixels(0, 0, Texture.m_Width, Texture.m_Height, GL_RGBA, GL_UNSIGNED_BYTE, pCommand->m_pResult->m_Image.m_pData);
...
void CCommandProcessorFragment_OpenGL3_3::Cmd_Draw(const CCommandBuffer::SCommand_Draw *pCommand)
{
	...
	if(pCommand->m_PrimitiveType == EPrimitiveType::QUADS && !m_BufferHandles.IsActive(pCommand->m_IndexBuffer))
		return;
...
void CCommandProcessorFragment_OpenGL3_3::SetState(const CCommandBuffer::SState &State, CGLSLTWProgram *pProgram, bool Use2DArrayTextures)
{
	...
	{
		const int Slot = 0;
		CTexture &Texture = m_vTextures[State.m_Texture.Id()];
		if(Use2DArrayTextures)
		{
			glBindTexture(GL_TEXTURE_2D_ARRAY, Texture.m_Tex2DArray);
			glBindSampler(Slot, Texture.m_Sampler2DArray);
		}
		else
		{
			glBindTexture(GL_TEXTURE_2D, Texture.m_Tex);
			glBindSampler(Slot, Texture.m_Sampler);
		}
		...
		if(Texture.m_LastWrapMode != State.m_WrapMode && !Use2DArrayTextures)
		{
			switch(State.m_WrapMode)
			{
			case EWrapMode::REPEAT:
				glSamplerParameteri(Texture.m_Sampler, GL_TEXTURE_WRAP_S, GL_REPEAT);
				glSamplerParameteri(Texture.m_Sampler, GL_TEXTURE_WRAP_T, GL_REPEAT);
				break;
			case EWrapMode::CLAMP:
				glSamplerParameteri(Texture.m_Sampler, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
				glSamplerParameteri(Texture.m_Sampler, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
				break;
			default:
				dbg_assert_failed("Invalid wrap mode: %d", (int)State.m_WrapMode);
			};
			Texture.m_LastWrapMode = State.m_WrapMode;
		}
	}
