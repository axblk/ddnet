...
{
	// OpenGL ES 3.0 and WebGL 2 only take ALPHA, LUMINANCE, LUMINANCE_ALPHA, RGB
	// and RGBA as an unsized internal format; RED and RG have to be sized. The
	// glyph atlas is RG8 and runs into exactly that, so the internal format is
	// picked here instead of reusing the one the pixels are given in.
	switch(Format)
	{
	case IGraphics::ETextureFormat::RGBA8_UNORM: return GL_RGBA;
	case IGraphics::ETextureFormat::RG8_UNORM: return GL_RG8;
	case IGraphics::ETextureFormat::R8_UNORM: return GL_R8;
	}
	dbg_assert(false, "Unknown texture format");
	dbg_break();
}
...
#if defined(CONF_PLATFORM_EMSCRIPTEN)
	// WebGL 2 has no buffer mapping of any kind. glMapBufferRange links here
	// only because Emscripten emulates it, and that emulation knows no read
	// access, so it hands back nothing and every exported frame is lost. The
	// pixels have to come across synchronously instead, the way the
	// presentation readback already takes them. The same reasoning about rows
	// as below applies: nothing is flipped here either.
