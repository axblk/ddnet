...
{
public:
	bool LoadShader(CGLSLCompiler *pCompiler, const char *pName, int Type);
...
// The backend with programs, for OpenGL 3.3 and OpenGL ES 3.
class CCommandProcessorFragment_OpenGL3_3 : public CCommandProcessorFragment_OpenGLBase
{
protected:
	...
	// A vertex array per (buffer, layout) pair, built on first use and kept
	// until the buffer goes away.
	struct SVertexArray
	{
	...
	// Compiles the two shaders with the defines, links them and makes the
	// program current.
	void LinkProgram(CGLSLTWProgram *pProgram, CGLSLCompiler &Compiler, const char *pVertexShader, const char *pFragmentShader, const std::vector<std::pair<const char *, std::string>> &vDefines = {});
	void InitPrimExProgram(CGLSLPrimitiveExProgram *pProgram, CGLSLCompiler &Compiler, bool Textured);
	...
	static int ToGLInternalFormat(IGraphics::ETextureFormat Format);
