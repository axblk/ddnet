...
void CCommandProcessorFragment_OpenGLBase::SetScissor(const CCommandBuffer::SState &State)
{
	if(State.m_ClipEnable)
	{
		if(m_RenderingToTexture)
		{
			// The clip is in the target's pixels, and the projection is flipped for a
			// render target, so the clip's top edge is the framebuffer's bottom edge.
	...
	*pCommand->m_pInitError = 0;
	...
	if(pCommand->m_RequestedBackend == BACKEND_TYPE_OPENGL)
	{
		...
		if(*pCommand->m_pInitError == 0)
		{
			...
			{
				// No programs: every pipeline is emulated on the CPU.
				...
				// Buffer objects are core from OpenGL 1.5 on.
				m_HasBufferObjects = MajorV > 1 || MinorV >= 5 || GLEW_ARB_vertex_buffer_object;

				// Layered textures are volumes here, and every map needs one.
				int Texture3DSize = 0;
	...
	else if(pCommand->m_RequestedBackend == BACKEND_TYPE_OPENGL_ES)
	{
		if(MajorV < 3)
		{
			// Layered textures need OpenGL ES 3, and every map needs one.
...
void CCommandProcessorFragment_OpenGLBase::DestroyTexture(int Slot)
{
	...
	// Only the backend with programs makes samplers.
...
ERunCommandReturnTypes CCommandProcessorFragment_OpenGLBase::RunCommand(const CCommandBuffer::SCommand *pBaseCommand)
{
	...
	if(!IsDrawCommandValid(pBaseCommand))
		return Handled;
	...
	case CCommandBuffer::CMD_SWAP:
		// Every frame ends in one, and the platform still has to swap.
