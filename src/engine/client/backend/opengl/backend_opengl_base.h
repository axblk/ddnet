...
// What every OpenGL backend shares: textures, handle bookkeeping, context
// probing and command dispatch.
...
protected:
	...
	// Where the presented image lies on the surface, bottom-left origin.
	...
	// What the context turned out to offer, filled in by InitOpenGL. Only the
	// fixed function path can be without them.
	...
	// For readbacks left to the device: the first runs once per frame, the
	// second when the frontend has to have a picture now.
