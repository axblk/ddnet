...
void main()
{
	...
	// GLSL ES only lets a fragment output be indexed by a constant, and a loop
	// counter is not one of those, so the four bytes are gathered here and
	// written out in one piece below.
	vec4 Bytes;
	for(int Component = 0; Component < 4; ++Component)
	{
		...
		{
			Bytes[Component] = Luma(texelFetch(gTextureSampler, ivec2(Byte, Target.y), 0).rgb);
			continue;
		}
		if(!Planar)
		{
			// One row of the plane holds both components, alternating.
			Bytes[Component] = (Byte & 1) == 0 ?
		...
		Bytes[Component] = Second ? ChromaRed(Block(Chroma)) : ChromaBlue(Block(Chroma));
	}
	FragClr = Bytes;
}
