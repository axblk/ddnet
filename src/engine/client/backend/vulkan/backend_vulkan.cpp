...
bool CCommandProcessorFragment_Vulkan::CollectGpuTimestamp(uint32_t ImageIndex)
{
	...
	const uint64_t TimeNanoseconds = TicksToNanoseconds(TimestampTickDelta(aTimestamps[0], aTimestamps[1], m_GpuTimestampValidBits), m_GpuTimestampPeriod);
	...
	for(uint32_t Interval = 0; Interval < IntervalCount; ++Interval)
	{
		...
		const uint64_t IntervalNanoseconds = TicksToNanoseconds(TimestampTickDelta(aTimestamps[Query], aTimestamps[Query + 1], m_GpuTimestampValidBits), m_GpuTimestampPeriod);
