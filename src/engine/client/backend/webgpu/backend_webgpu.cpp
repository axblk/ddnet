...
namespace
{
	...
	class CCommandProcessorFragment_WebGpu final : public CCommandProcessorFragment_Renderer
	{
		...
		std::array<WGPUSampler, 2> m_aSamplers{};
		...
		bool ProcessUntilDone(const T &Result, const char *pOperation)
		{
			...
			while(!Result.m_Done && std::chrono::steady_clock::now() < Deadline)
			{
		...
		bool CreateMultisampleTarget(WGPUTextureFormat Format, uint32_t Width, uint32_t Height, WGPUTexture &Texture, WGPUTextureView &View, size_t &MemorySize)
		{
		...
		bool CreateTextureBindGroups(WGPUTextureView View, WGPUBindGroupLayout Layout, uint32_t TextureBinding, std::array<WGPUBindGroup, 2> &aBindGroups)
		{
		...
		bool CreateBufferedPipelines(std::array<WGPURenderPipeline, BUFFERED_PIPELINE_COUNT> &aPipelines, WGPUTextureFormat Format, const char *pVertexEntry, bool Instanced)
		{
		...
		bool CreateLayeredPrimitivePipelines(SPipelineSet &Pipelines, WGPUTextureFormat Format)
		{
		...
		bool CreateArrayColorPipelines(std::array<WGPURenderPipeline, BUFFERED_PIPELINE_COUNT> &aPipelines, WGPUTextureFormat Format, bool Transform)
		{
			for(size_t Blend = 0; Blend < BLEND_MODE_COUNT; ++Blend)
			{
				for(size_t Textured = 0; Textured < 2; ++Textured)
		...
		bool CreateQuadPipelines(std::array<WGPURenderPipeline, BUFFERED_PIPELINE_COUNT> &aPipelines, WGPUTextureFormat Format, bool Shared)
		{
			for(size_t Blend = 0; Blend < BLEND_MODE_COUNT; ++Blend)
			{
				for(size_t Textured = 0; Textured < 2; ++Textured)
		...
		bool CreateDualAtlasPipelines(SPipelineSet &Pipelines, WGPUTextureFormat Format)
		{
			std::array<WGPUVertexAttribute, 3> aAttributes{};
			aAttributes[0].format = WGPUVertexFormat_Float32x2;
		...
		bool CreatePipelineSet(SPipelineSet &Pipelines, WGPUTextureFormat Format)
		{
			...
			if(Pipelines.m_Blur == nullptr)
				return false;
			return CreateBufferedPipelines(Pipelines.m_aUniformColor, Format, "vs_uniform_color", false) &&
			       CreateBufferedPipelines(Pipelines.m_aInstanced, Format, "vs_instanced", true) &&
			       CreateLayeredPrimitivePipelines(Pipelines, Format) &&
			       CreateArrayColorPipelines(Pipelines.m_aArrayColor, Format, false) &&
			       CreateArrayColorPipelines(Pipelines.m_aArrayColorTransform, Format, true) &&
			       CreateQuadPipelines(Pipelines.m_aQuadPerItem, Format, false) &&
			       CreateQuadPipelines(Pipelines.m_aQuadShared, Format, true) &&
			       CreateDualAtlasPipelines(Pipelines, Format);
		}

		bool CreatePrimitivePipelines()
		{
			return CreatePipelineSet(m_aPipelineSets[0], m_SurfaceFormat) && CreatePipelineSet(m_aPipelineSets[1], WGPUTextureFormat_RGBA8Unorm);
		}
...
@fragment fn fs_dual_atlas(input: VertexOutput) -> @location(0) vec4f {
	...
	return vec4f(0.0);
			...
			for(size_t i = 0; i < m_aSamplers.size(); ++i)
			{
				...
				SamplerDescriptor.addressModeU = i == 0 ? WGPUAddressMode_Repeat : WGPUAddressMode_ClampToEdge;
				SamplerDescriptor.addressModeV = SamplerDescriptor.addressModeU;
				SamplerDescriptor.addressModeW = SamplerDescriptor.addressModeU;
			...
			BufferDescriptor.size = STREAM_BUFFER_SIZE;
			...
			BufferDescriptor.size = UNIFORM_BUFFER_SIZE;
			...
			if(m_StreamBuffer == nullptr || m_UniformBuffer == nullptr)
				return false;
			m_pStreamMemoryUsage->fetch_add(STREAM_BUFFER_SIZE + UNIFORM_BUFFER_SIZE, std::memory_order_relaxed);
		...
		bool CreateTexture(const CCommandBuffer::SCommand_Texture_Create *pCommand)
		{
			...
			auto CreateNativeTexture = [&](WGPUTexture &NativeTexture, WGPUTextureView &View, std::array<WGPUBindGroup, 2> &aBindGroups, uint32_t Width, uint32_t Height, uint32_t Layers, WGPUBindGroupLayout Layout, uint32_t TextureBinding, const uint8_t *pData) {
				...
				ViewDescriptor.dimension = Layers == 1 ? WGPUTextureViewDimension_2D : WGPUTextureViewDimension_2DArray;
				...
				if(View == nullptr || !CreateTextureBindGroups(View, Layout, TextureBinding, aBindGroups))
					return false;
		...
		bool EnsureRenderPass()
		{
			...
			if(SampleCount() != 1)
			{
				...
				if(!CreateMultisampleTarget(pTarget != nullptr ? WGPUTextureFormat_RGBA8Unorm : m_SurfaceFormat, Width, Height, MultisampleTexture, MultisampleView, MultisampleMemorySize))
					return false;
				RenderView = MultisampleView;
			}
			WGPURenderPassColorAttachment ColorAttachment = WGPU_RENDER_PASS_COLOR_ATTACHMENT_INIT;
			ColorAttachment.view = RenderView;
			ColorAttachment.resolveTarget = SampleCount() == 1 ? nullptr : ResolveView;
		...
		bool ApplyState(const CCommandBuffer::SState &State, WGPURenderPipeline Pipeline, const ColorRGBA &Color, const vec2 &RotationCenter, float Rotation, bool TextureArray = false, const vec2 &VertexOffset = vec2(0.0f, 0.0f), const vec2 &VertexScale = vec2(1.0f, 1.0f), uint32_t QuadBase = 0, float TextureSize = 1.0f, const ColorRGBA &SecondaryColor = ColorRGBA(1.0f, 1.0f, 1.0f, 1.0f))
		{
			...
			const uint32_t UniformOffset = static_cast<uint32_t>(m_UniformOffset);
		...
		bool WriteQuadTransforms(const CCommandBuffer::SDrawDataQuadTransform *pData, uint32_t Count, uint32_t &Offset)
		{
			...
			Offset = static_cast<uint32_t>(m_UniformOffset);
		...
		bool QuerySurfaceConfiguration()
		{
			...
			if(!Complete || !m_SupportsFifo || m_SurfaceFormat == WGPUTextureFormat_Undefined)
				return false;
			if(m_PrimitiveShader != nullptr && PreviousFormat != WGPUTextureFormat_Undefined && PreviousFormat != m_SurfaceFormat && !CreatePipelineSet(m_aPipelineSets[0], m_SurfaceFormat))
				return false;
		...
		bool SubmitCommands()
		{
			...
			if(m_StreamOffset != 0)
				wgpuQueueWriteBuffer(m_Queue, m_StreamBuffer, 0, m_vStreamUpload.data(), m_StreamOffset);
			if(m_UniformOffset != 0)
				wgpuQueueWriteBuffer(m_Queue, m_UniformBuffer, 0, m_vUniformUpload.data(), m_UniformOffset);
