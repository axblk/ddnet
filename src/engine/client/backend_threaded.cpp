...
bool GraphicsBackendDriverVersion(EBackendType BackendType, EGraphicsDriverAgeType DriverAgeType, int &Major, int &Minor, int &Patch, const char *&pName)
{
	...
	{
		pName = "GLES";
		// Only one version to offer: there is no ES below 3.0 to run. ES 1.x is
		// fixed function, which this client has not drawn with since it moved to
		// programs, and the window forces the version to 3.0 anyway - offering
		// an older one would name a backend that nobody can get.
