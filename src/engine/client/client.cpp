...
int CClient::SendMsg(int Conn, CMsgPacker *pMsg, int Flags)
{
	...
	CPacker Pack;
	if(!RepackMsg(pMsg, Pack, IsSixup(m_NetworkSessionId)))
		return 0;
	...
	if((Flags & MSGFLAG_RECORD) && Conn == ActiveConnection())
	{
...
void CClient::SendInput()
{
	...
	if(Connection(ActiveConn).m_PredTick <= 0)
		return;

	bool Force = false;
	// fetch input
	const int aInputConnections[] = {
		ActiveConn,
		ActiveConn == CONN_MAIN ? CONN_DUMMY : CONN_MAIN,
	};
	for(const int Conn : aInputConnections)
	{
		if(!DummyConnected() && Conn != ActiveConn)
		{
			break;
		}
		...
		if(GameConnection.m_PredTick <= 0)
			continue;
		int Size = GameClient()->OnSnapInput(m_NetworkSessionId, GameConnection.m_aInputs[GameConnection.m_CurrentInput].m_aData, Conn, Force);
		...
		if(Size)
		{
			...
			for(int k = 0; k < Size / 4; k++)
			{
				...
				if(k == FlagsOffset && IsSixup(m_NetworkSessionId))
				{
...
void CClient::StopNetworkSession(const char *pReason)
{
	...
	m_LastActiveConnection = CONN_MAIN;
...
{
	const bool InactiveStream = Conn != ActiveConnection();
	CUnpacker Unpacker;
	...
	if(Sys)
	{
		...
		else if(Conn == CONN_MAIN && (pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_CAPABILITIES)
		{
			...
			m_ServerCapabilities = GetServerCapabilities(Version, Flags, IsSixup(m_NetworkSessionId));
		...
		else if(Conn == CONN_MAIN && (pPacket->m_Flags & NET_CHUNKFLAG_VITAL) != 0 && Msg == NETMSG_MAP_CHANGE)
		{
			if(m_CanReceiveServerCapabilities)
			{
				m_ServerCapabilities = GetServerCapabilities(0, 0, IsSixup(m_NetworkSessionId));
				m_CanReceiveServerCapabilities = false;
			}
		...
		else if(Conn == CONN_MAIN && Msg == NETMSG_MAP_DATA)
		{
			...
			else
			{
				...
				if(IsSixup(m_NetworkSessionId) && (m_MapdownloadChunk % TranslationContext.m_MapDownloadChunksPerRequest == 0))
				{
					CMsgPacker MsgP(protocol7::NETMSG_REQUEST_MAP_DATA, true, true);
					SendMsg(CONN_MAIN, &MsgP, MSGFLAG_VITAL | MSGFLAG_FLUSH);
				}
		...
		else if(Msg == NETMSG_INPUTTIMING)
		{
			...
			for(int k = 0; k < 200; k++)
			{
				if(Connection(Conn).m_aInputs[k].m_Tick == InputPredTick)
				{
					Target = Connection(Conn).m_aInputs[k].m_PredictedTime + (Now - Connection(Conn).m_aInputs[k].m_Time);
					...
					if(!InactiveStream)
					{
						...
						if(DemoSnapSize >= 0)
						{
							...
							for(auto &DemoRecorder : DemoRecorders())
							{
								...
								{
									// write snapshot
									DemoRecorder.RecordSnapshot(GameTick, IsSixup(m_NetworkSessionId) ? SnapSeven.AsSnapshot() : TmpBuffer3.AsSnapshot(), DemoSnapSize);
								}
							}
						}
					}
...
void CClient::PumpNetwork()
{
	...
	if(m_pNetworkSessionSource->State() != ESessionState::OFFLINE && m_State < IClient::STATE_QUITTING)
	{
	...
	if(m_pNetworkSessionSource->State() == ESessionState::CONNECTING && NetClient(CONN_MAIN).State() == NETSTATE_ONLINE)
	{
		...
		{
			GameClient()->OnSessionClosed(m_NetworkSessionId);
			m_pNetworkSessionSource->SetState(ESessionState::LOADING_MAP);
		}
		if(m_SessionManager.FocusedId() == m_NetworkSessionId)
	...
	for(int Conn = 0; Conn < NUM_CONNS; Conn++)
	{
		while(NetClient(Conn).Recv(&Packet, &ResponseToken, IsSixup(m_NetworkSessionId)))
		{
...
void CClient::UpdateNetworkSession()
{
	...
	if(m_pNetworkSessionSource->State() == ESessionState::READY)
	{
		...
		if(OtherRepredict && OtherGameConnection.m_PredTick > OtherGameConnection.m_CurGameTick && OtherGameConnection.m_PredTick < OtherGameConnection.m_CurGameTick + MaxLatencyTicks())
			GameClient()->OnPredict(m_NetworkSessionId, OtherConn);
		if(ActiveRepredict && ActiveGameConnection.m_PredTick > ActiveGameConnection.m_CurGameTick && ActiveGameConnection.m_PredTick < ActiveGameConnection.m_CurGameTick + MaxLatencyTicks())
			GameClient()->OnPredict(m_NetworkSessionId, ActiveConn);
