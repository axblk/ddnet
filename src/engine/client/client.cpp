...
{
	return &m_pNetworkSessionSource->SnapshotDelta(m_pNetworkSessionSource->m_Sixup);
}
	...
	m_FpsGraph(4096, 0, true)
{
	...
	m_pDummySessionSource->m_pServerSource = m_pNetworkSessionSource;
	...
	auto pVideoExportSource = std::make_unique<CDemoSessionSource>(true, [this]() { UpdateDemoIntraTimers(m_VideoExportSessionId); });
	...
	m_VideoExportDemoListener = CDemoListener(this, m_VideoExportSessionId);
	m_pVideoExportSessionSource->m_DemoPlayer.SetListener(&m_VideoExportDemoListener);
#endif
...
{
	if(Connection(Dummy).m_RconAuthed != 0)
		return;
...
void CClient::StopDemoRecorders()
{
...
bool CClient::ConnectionProblems(CSessionId SessionId) const
{
	if(SessionId != m_NetworkSessionId && SessionId != m_DummySessionId)
		return false;
	const int Conn = SessionId == m_DummySessionId ? CONN_DUMMY : CONN_MAIN;
	...
	return m_aNetClient[Conn].GotProblems(MaxLatency);
}
...
void CClient::SendInput()
{
	...
	for(int Dummy = 0; Dummy < NUM_DUMMIES; Dummy++)
	{
		...
		if(GameConnection.m_PredTick <= 0 || (Dummy != 0 && m_aNetClient[Conn].State() != NETSTATE_ONLINE))
		{
			if(Dummy == 0)
				return;
			continue;
		}
		CConnection::CInput &Input = GameConnection.m_aInputs[GameConnection.m_CurrentInput];
		const int Size = GameClient()->OnSnapInput(SeatSessionId(Conn), Input.m_aData, Force);
		if(!Size)
			continue;
		...
		Input.m_Tick = GameConnection.m_PredTick;
		Input.m_PredictedTime = GameConnection.m_PredictedTime.Get(Now);
		Input.m_PredictionMargin = PredictionMargin() * time_freq() / 1000;
		Input.m_Time = Now;
		...
		for(int k = 0; k < Size / 4; k++)
		{
			...
			if(k == FlagsOffset && m_pNetworkSessionSource->m_Sixup)
				Msg.AddInt(PlayerFlags_SixToSeven(Input.m_aData[k]));
			else
				Msg.AddInt(Input.m_aData[k]);
		}
...
{
	Connection(Conn).m_DidPostConnect = false;
	...
	if(Conn == CONN_MAIN)
	{
		m_pNetworkSessionSource->m_NextPingTime = time_get() + time_freq() / 2;
...
void CClient::GenerateTimeoutCodes(const NETADDR *pAddrs, int NumAddrs)
{
	...
	for(int Dummy = 0; Dummy < NUM_DUMMIES; Dummy++)
	{
		char (&aTimeoutCode)[32] = Connection(Dummy).m_aTimeoutCode;
		GenerateTimeoutCode(aTimeoutCode, sizeof(aTimeoutCode), g_Config.m_ClTimeoutSeed, pAddrs, NumAddrs, Dummy);
		log_debug("client", "timeout code '%s' (%s)", aTimeoutCode, Dummy == 0 ? "normal" : "dummy");
	}
}
...
void CClient::StartLegacyConnection(const NETADDR *pAddrs, int NumAddrs, bool Sixup)
{
	...
	if(Sixup)
		m_aNetClient[CONN_MAIN].Connect7(pAddrs, NumAddrs);
	else
		m_aNetClient[CONN_MAIN].Connect(pAddrs, NumAddrs);
...
bool CClient::TryStartModernTransport(const CConnectTarget &Target)
{
	...
	m_aNetClient[CONN_MAIN].SetPacketFilter(
...
void CClient::Connect(const char *pAddress, const char *pPassword)
{
	...
	{
		// Servers ask for a reconnect or a redirect from within the session
		// update, which the session has to leave before it can stop.
		Source.ScheduleServerConnect(pAddress, pPassword);
		StopSession(m_NetworkSessionId, nullptr);
		return;
	}
	...
	const bool Valid = Target.Parse(Source.m_ConnectAddress.c_str(), m_aNetClient[CONN_MAIN].NetType(), (EConnectAddressFamily)g_Config.m_ClConnectAddressFamily);
#if defined(CONF_PLATFORM_EMSCRIPTEN)
...
{
	SessionSource(SessionId).RequestStop(pReason);
	if(m_UpdatingSessionId != SessionId)
		FinishStopSession(SessionId);
}

void CClient::FinishStopSession(CSessionId SessionId)
{
	CSessionSource &Source = SessionSource(SessionId);
	if(Source.State() != ESessionState::STOPPING)
		return;
	const std::string Reason = Source.TakeStopReason();
	const char *pReason = Reason.empty() ? nullptr : Reason.c_str();
	if(SessionId == m_DummySessionId)
		DummyDisconnect(pReason);
	else if(Source.Type() == ESessionSourceType::NETWORK)
		StopNetworkSession(pReason);
	else
		StopDemoSession(SessionId, pReason);
	Source.SetState(ESessionState::OFFLINE);
}

void CClient::UpdateSessions()
{
	for(const auto &pSource : m_SessionManager.Sessions())
	{
		const CSessionId SessionId = pSource->Id();
		// The dummy is updated with the server it plays on.
		if(SessionId == m_DummySessionId)
			continue;
		m_UpdatingSessionId = SessionId;
		if(pSource->Type() == ESessionSourceType::NETWORK)
			UpdateNetworkSession();
		else
			UpdateDemoSession(SessionId);
		m_UpdatingSessionId = CSessionId();
		FinishStopSession(SessionId);
	}
}
...
void CClient::StopNetworkSession(const char *pReason)
{
	...
	str_copy(aReconnectError, m_aNetClient[CONN_MAIN].ErrorString());
	const bool Focused = FocusedSessionId() == m_NetworkSessionId;
		...
		m_QuicTransport.Close(m_QuicSession, pReason ? pReason : "application disconnect"))
	{
		...
		{
			m_aNetClient[CONN_MAIN].Update();
			CNetChunk Packet;
			SECURITY_TOKEN ResponseToken;
			while(m_aNetClient[CONN_MAIN].Recv(&Packet, &ResponseToken, m_pNetworkSessionSource->m_Sixup))
			{
			}
			NETADDR Address;
			unsigned char *pData;
			...
			while((DataSize = m_QuicTransport.PollUdpSend(&Address, &pData)) > 0)
				m_aNetClient[CONN_MAIN].SendRaw(&Address, pData, DataSize);
			CQuicEvent Event;
	...
	// clear snapshots
	Connection(CONN_MAIN).ResetSnapshots();
}
...
{
	StopSession(FocusedSessionId(), nullptr);
}
...
void CClient::DummyConnect()
{
	...
	m_pDummySessionSource->SetState(ESessionState::CONNECTING);
	// connect to the server
	if(m_pNetworkSessionSource->m_Sixup)
...
void CClient::DummyDisconnect(const char *pReason)
{
	...
	m_pDummySessionSource->SetState(ESessionState::OFFLINE);
...
{
	m_pNetworkSessionSource->m_ServerInfo = {};
	m_CurrentServerInfoRequestTime = 0;
}

void CClient::SetCurrentServerInfo(const CServerInfo &ServerInfo)
{
	CServerInfo &CurrentServerInfo = m_pNetworkSessionSource->m_ServerInfo;
	CurrentServerInfo = ServerInfo;
	m_CurrentServerInfoRequestTime = -1;
	const IMap *pMap = GameClient()->Map(m_NetworkSessionId);
...
void CClient::RenderDebug()
{
	...
	str_format(aBuffer, sizeof(aBuffer), "Game/predicted tick: %d/%d", GameTick(SessionId), PredGameTick(SessionId));
	Graphics()->QuadsText(2, 2, FontSize, aBuffer);

	str_format(aBuffer, sizeof(aBuffer), "Prediction time: %d ms", GetPredictionTime(SessionId));
	...
	{
		// In the order the game registered the zones, as far as the line goes.
		str_copy(aBuffer, "GPU zones:");
		const std::span<const std::string> vZoneNames = Graphics()->GpuRenderZoneNames();
		for(size_t Zone = 0; Zone < vZoneNames.size(); ++Zone)
		{
			if((RenderStats.m_GpuRenderZoneMask & (1U << Zone)) == 0)
				continue;
			char aZone[64];
			str_format(aZone, sizeof(aZone), " %s %.3f", vZoneNames[Zone].c_str(), RenderStats.m_aGpuRenderZoneNanoseconds[Zone] / 1000000.0);
			str_append(aBuffer, aZone);
		}
	...
	// Snapshots
	{
		...
		{
			if(SnapshotDelta()->GetDataRate(i) && Connection(g_Config.m_ClDummy).m_apSnapshots[ISessions::SNAP_CURRENT])
			{
				const int Type = Connection(g_Config.m_ClDummy).m_apSnapshots[ISessions::SNAP_CURRENT]->m_pAltSnap->GetExternalItemType(i);
...
void CClient::RenderGraphs()
{
	...
	Connection(g_Config.m_ClDummy).m_InputtimeMarginGraph.Scale(5 * time_freq());
	Connection(g_Config.m_ClDummy).m_InputtimeMarginGraph.Render(Graphics(), TextRender(), GraphX, GraphSpacing * 6 + GraphH, GraphW, GraphH, "Prediction Margin");
	Connection(g_Config.m_ClDummy).m_GametimeMarginGraph.Scale(5 * time_freq());
	Connection(g_Config.m_ClDummy).m_GametimeMarginGraph.Render(Graphics(), TextRender(), GraphX, GraphSpacing * 7 + GraphH * 2, GraphW, GraphH, "Gametime Margin");
}
...
{
	if(!m_aNetClient[CONN_MAIN].SocketIsBroken() && !m_aNetClient[CONN_DUMMY].SocketIsBroken() && !m_aNetClient[CONN_CONTACT].SocketIsBroken())
	{
		return;
	}
...
#define GET_INT(integer) (integer) = str_toint(Up.GetString())
	...
	if(!Up.Error() || IgnoreError)
	{
		...
		CNetworkSessionSource &Source = *m_pNetworkSessionSource;
		if((m_QuicConnected || m_aNetClient[CONN_MAIN].State() == NETSTATE_ONLINE) &&
			ServerAddress() == *pFrom &&
			RawType != SERVERINFO_EXTENDED_MORE)
		{
			...
			// us.
			if(SavedType >= Source.m_ServerInfo.m_Type &&
				GameClient()->Map(m_NetworkSessionId)->IsLoaded())
			{
				SetCurrentServerInfo(Info);
				Discord()->UpdateServerInfo(Source.m_ServerInfo);
			}
...
void CClient::ProcessServerPacket(CNetChunk *pPacket, int Conn, bool Dummy)
{
	...
	if(Sys)
	{
		...
		else if(Conn == CONN_MAIN && Vital && Msg == NETMSG_CAPABILITIES)
		{
			...
			Source.m_ServerCapabilities = GetServerCapabilities(Version, Flags, Source.m_Sixup);
		...
		else if(Conn == CONN_MAIN && Vital && Msg == NETMSG_MAP_CHANGE)
		{
			if(Source.m_CanReceiveServerCapabilities)
			{
				Source.m_ServerCapabilities = GetServerCapabilities(0, 0, Source.m_Sixup);
				Source.m_CanReceiveServerCapabilities = false;
			}
			...
			// The map is loaded for the network session; the dummy that stays
			// on the server loses its snapshots of the old map with it.
			Connection(CONN_DUMMY).ResetSnapshots();
			if(m_pDummySessionSource->State() == ESessionState::READY)
				m_pDummySessionSource->SetState(ESessionState::LOADING_MAP);
		...
		else if(Conn == CONN_MAIN && Msg == NETMSG_MAP_DATA)
		{
			...
			int Chunk = -1;
			int Size = -1;
			CTranslationContext &TranslationContext = Source.m_TranslationContext;
			...
			else
			{
				...
				if(Source.m_Sixup && (Source.m_MapdownloadChunk % TranslationContext.m_MapDownloadChunksPerRequest == 0))
				{
					CMsgPacker MsgP(protocol7::NETMSG_REQUEST_MAP_DATA, true, true);
					SendMsg(CONN_MAIN, &MsgP, MSGFLAG_VITAL | MSGFLAG_FLUSH);
				}
		...
		else if(Conn == CONN_MAIN && Vital && Msg == NETMSG_MAP_RELOAD)
		{
			if(m_DummyConnected)
			{
				...
				m_DummyDeactivateOnReconnect = g_Config.m_ClDummy == CONN_MAIN;
				g_Config.m_ClDummy = 0;
			}
			else
			{
		...
		else if(Conn == CONN_MAIN && Vital && Msg == NETMSG_CON_READY)
		{
			...
			GameClient()->OnConnected(m_NetworkSessionId);
		...
		else if(Msg == NETMSG_RECONNECT)
		{
			if(Conn == CONN_MAIN)
			{
				...
				Connect(ConnectAddress.c_str(), Password.c_str());
			}
			else
			{
		...
		else if(Vital && Msg == NETMSG_RCON_AUTH_STATUS)
		{
			...
			if(!Unpacker.Error())
			{
				...
				if(GameConnection.m_RconAuthed)
					RconAuth(m_aRconUsername, m_aRconPassword, g_Config.m_ClDummy ^ 1);
			}
			if(Conn == CONN_MAIN)
			{
				int Old = Source.m_UseTempRconCommands;
		...
		else if(Msg == NETMSG_SNAP || Msg == NETMSG_SNAPSINGLE || Msg == NETMSG_SNAPEMPTY)
		{
			...
			if(GameTick >= GameConnection.m_CurrentRecvTick && GameTick > GameConnection.m_AckGameTick)
			{
					...
					(NumParts == CSnapshot::MAX_PARTS && GameConnection.m_SnapshotParts == std::numeric_limits<uint64_t>::max()))
				{
					...
					{
						int IntSize = CVariableInt::Decompress(GameConnection.m_aSnapshotIncomingData, GameConnection.m_SnapshotIncomingDataSize, TmpBuffer2.m_aData, sizeof(TmpBuffer2.m_aData));
					...
					if(Source.m_Sixup)
					{
						...
						AltSnapSize = GameClient()->TranslateSnap(SeatSessionId(Conn), &AltSnapBuffer, TmpTransSnapBuffer.AsSnapshot());
					}
					else
					{
					...
					if(!Dummy)
					{
						...
						if(Source.m_Sixup)
						{
							DemoSnapSize = GameClient()->OnDemoRecSnap7(SeatSessionId(Conn), TmpBuffer3.AsSnapshot(), &SnapSeven);
							if(DemoSnapSize < 0)
							{
						...
						if(DemoSnapSize >= 0)
						{
							...
							for(auto &DemoRecorder : DemoRecorders())
							{
								...
								{
									// write snapshot
									DemoRecorder.RecordSnapshot(GameTick, Source.m_Sixup ? SnapSeven.AsSnapshot() : TmpBuffer3.AsSnapshot(), DemoSnapSize);
								}
							}
						}
					}
					...
					if(GameConnection.m_ReceivedSnapshots == 2)
					{
						...
						GameConnection.m_PredictedTime.UpdateMargin(PredictionMargin() * time_freq() / 1000);
						...
						GameClient()->OnNewSnapshot(SeatSessionId(Conn));
						if(Conn == CONN_DUMMY)
							m_pDummySessionSource->SetState(ESessionState::READY);
						else if(FocusedSessionId() == m_NetworkSessionId)
	...
	else if(Vital || Msg == NETMSGTYPE_SV_PREINPUT)
	{
		...
		GameClient()->OnMessage(SeatSessionId(Conn), Msg, &Unpacker);
	}
}
...
void CClient::FinishMapDownload()
{
	...
	StopDemoRecorders();
	const char *pError = LoadMap(m_NetworkSessionId, Source.m_aMapdownloadName, Source.m_aMapdownloadFilename, Source.m_MapdownloadSha256, Source.m_MapdownloadCrc);
	if(!pError)
	{
		ResetMapDownload(true);
		m_pConsole->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "client/network", "loading done");
...
void CClient::PumpNetwork()
{
	...
	while((QuicDataSize = m_QuicTransport.PollUdpSend(&QuicAddress, &pQuicData)) > 0)
		m_aNetClient[CONN_MAIN].SendRaw(&QuicAddress, pQuicData, QuicDataSize);
	...
	const bool MainOnline = m_UseQuic ? m_QuicConnected : m_aNetClient[CONN_MAIN].State() == NETSTATE_ONLINE;
	...
	for(int Conn = 0; Conn < NUM_CONNS; Conn++)
	{
		while(m_aNetClient[Conn].Recv(&Packet, &ResponseToken, Source.m_Sixup))
		{
...
void CClient::UpdateDemoSession(CSessionId SessionId)
{
	...
	if(Source.State() == ESessionState::READY && !UpdateDemoPlayer(SessionId))
	{
		...
		StopSession(SessionId, Player.ErrorMessage());
		if(Player.ErrorMessage()[0] != '\0' && FocusedSessionId() == SessionId)
		{
...
void CClient::UpdateNetworkSession()
{
	...
	if(Source.State() == ESessionState::READY)
	{
		...
		const bool DummySwapped = m_LastDummy != ActiveConn;
		if(DummySwapped && FocusedSessionId() == m_NetworkSessionId)
		{
			// Invalidate references to the snapshots of the other connection
			GameClient()->InvalidateSnapshot(m_NetworkSessionId);
			GameClient()->OnDummySwap();
		}

		const int64_t ClockNow = time_get();
		bool aRepredict[NUM_DUMMIES] = {false, false};
		bool SendNewInput = false;
		for(const int Conn : {ActiveConn, ActiveConn ^ 1})
		{
			CConnection &GameConnection = Connection(Conn);
			...
			if(Conn == ActiveConn && DummySwapped && GameConnection.m_apSnapshots[SNAP_PREV])
			{
				GameClient()->OnNewSnapshot(SeatSessionId(Conn));
				aRepredict[Conn] = true;
			}
			...
			if(GameConnection.m_apSnapshots[SNAP_PREV])
			{
				...
				if(absolute(NewPredTick - GameConnection.m_apSnapshots[SNAP_PREV]->m_Tick) > MaxLatencyTicks())
				{
		...
		// Only the seat that gets the input is predicted, and only while the
		// server is in focus; the other sessions are drawn from their snapshots.
		// Only sane predictions are done.
		const CConnection &InputConnection = Connection(ActiveConn);
		if(FocusedSessionId() == m_NetworkSessionId && aRepredict[ActiveConn] && InputConnection.m_PredTick > InputConnection.m_CurGameTick && InputConnection.m_PredTick < InputConnection.m_CurGameTick + MaxLatencyTicks())
			GameClient()->OnPredict(SeatSessionId(ActiveConn));

		if(Connection(ActiveConn).m_apSnapshots[SNAP_CURRENT])
		{
			// fetch server info if we don't have it
			if(m_CurrentServerInfoRequestTime >= 0 && ClockNow > m_CurrentServerInfoRequestTime)
			{
			...
			if(Source.m_NextPingTime >= 0 && ClockNow > Source.m_NextPingTime)
			{
				...
				{
					m_ServerBrowser.RequestCurrentServerWithRandomToken(ServerAddress(), &Source.m_PingBasicToken, &Source.m_PingToken);
				}
				else
				{
				...
				Source.m_NextPingTime = ClockNow + 600 * time_freq(); // ping every 10 minutes
			}
		}
...
void CClient::Update()
{
	...
	// holds the video, so this is where a finished one is let go. A queued
	// export still has to read its status, so the queue lets go of its own.
	if(m_pVideo && m_pVideo->IsStopped() && !m_ActiveVideoExport.has_value())
		m_pVideo.reset();
#endif
	...
	std::string Password;
	if(Source.ConsumePendingConnect(ConnectAddress, Password))
	{
		Connect(ConnectAddress.c_str(), Password.c_str());
	}
	else if(Source.ConsumeReconnect(time_get()))
	{
		ConnectAddress = Source.m_ConnectAddress;
		Password = Source.m_SendPassword ? g_Config.m_Password : Source.m_Password;
		Connect(ConnectAddress.c_str(), Password.c_str());
	}
...
void CClient::Run()
{
...
#if defined(CONF_VIDEORECORDER)
	// init the video recorder: FFmpeg, or WebCodecs in a browser
	InitVideoBackend();
#endif
	...
	while(true)
	{
		set_new_tick();
#if defined(CONF_VIDEORECORDER)
		if(m_CommandLineVideoExport && VideoExportInterrupted())
		{
			...
			if(m_pVideo && IVideo::Current() == m_pVideo.get())
			{
				...
				StopSession(m_VideoSessionId, m_aVideoExportQueueError);
			else
		...
		// render
		{
				...
				RenderDue)
			{
				...
				if(VideoFrameHandled)
				{
					...
					if(pVideo->HasAudio())
					{
						...
						pVideo->NextAudioFrameTimeline([this, OfflineAudio](short *pFinalOut, unsigned Frames) {
							Sound()->Mix(pFinalOut, Frames, OfflineAudio);
						});
					}
					GameClient()->OnRenderFinalize();
				}
				...
				if(m_BenchmarkFile)
				{
					...
					str_format(aBuf, sizeof(aBuf),
						"Frametime %d us RenderWall %" PRIu64 " us GpuTime %" PRIu64 " us GpuZoneMask %u GpuSample %" PRIu64 " GpuSupported %d Commands %" PRIu64 " ResourceCommands %" PRIu64 " DrawCommands %" PRIu64 " DrawCalls %" PRIu64 " Triangles %" PRIu64 " Instances %" PRIu64 " RenderPasses %" PRIu64 " BufferCreates %" PRIu64 " BufferRecreates %" PRIu64 " TextureCreates %" PRIu64 " TextureUpdates %" PRIu64 " UploadBytes %" PRIu64 " StreamedBytes %" PRIu64 " TextLayout %" PRIu64 " us TextLayoutCalls %" PRIu64 " Glyphs %" PRIu64 " TextCreates %" PRIu64 " TextSoftRecreates %" PRIu64 " TextDeletes %" PRIu64 " TextRenders %" PRIu64 " TextUploadBytes %" PRIu64 " FramesProduced %" PRIu64 " FramesRendered %" PRIu64 " FramesDropped %" PRIu64 " TextureMemory %" PRIu64 " BufferMemory %" PRIu64 " StreamedMemory %" PRIu64 " StagingMemory %" PRIu64,
						...
						RenderStats.m_BufferCreates, RenderStats.m_BufferRecreates, RenderStats.m_TextureCreates, RenderStats.m_TextureUpdates, RenderStats.m_UploadBytes, RenderStats.m_StreamedBytes,
					...
					// Then the time of every zone measured, named as the game registered it.
					const std::span<const std::string> vZoneNames = Graphics()->GpuRenderZoneNames();
					for(size_t Zone = 0; Zone < vZoneNames.size(); ++Zone)
					{
						if((RenderStats.m_GpuRenderZoneMask & (1U << Zone)) == 0)
							continue;
						char aZone[128];
						str_format(aZone, sizeof(aZone), " Gpu:%s %" PRIu64 " us", vZoneNames[Zone].c_str(), RenderStats.m_aGpuRenderZoneNanoseconds[Zone] / 1000);
						str_append(aBuf, aZone);
					}
					str_append(aBuf, "\n");
			...
			if(RefreshRate)
			{
				...
				{
					const CConnection &PredictionConnection = Connection(g_Config.m_ClDummy);
	...
	StopSession(m_DemoSessionId, nullptr);
#if defined(CONF_VIDEORECORDER)
	StopSession(m_VideoExportSessionId, nullptr);
	if(m_pVideo)
	{
...
void CClient::Con_DbgDumpSessions(IConsole::IResult *, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	char aCount[32];
	str_format(aCount, sizeof(aCount), "sessions=%d", (int)pSelf->m_SessionManager.Sessions().size());
	pSelf->m_pConsole->Print(IConsole::OUTPUT_LEVEL_STANDARD, "client/session", aCount);
	for(const auto &pSource : pSelf->m_SessionManager.Sessions())
	{
		const CSessionId SessionId = pSource->Id();
		const int Seat = pSelf->Seats().SeatOf(SessionId);
		char aBuf[192];
		str_format(aBuf, sizeof(aBuf), "session=%" PRIu64 " type=%d state=%d seat=%d input=%d tick=%d map=%s", SessionId.Value(), static_cast<int>(pSource->Type()), static_cast<int>(pSource->State()), Seat, SessionId == pSelf->InputSessionId(), pSelf->GameTick(SessionId), pSelf->ServerInfo(SessionId).m_aMap);
...
CVideoExportSettings CClient::DefaultVideoExportSettings()
{
	...
	const int ScreenWidth = HasScreen ? Graphics()->ScreenWidth() : g_Config.m_GfxScreenWidth;
	const int ScreenHeight = HasScreen ? Graphics()->ScreenHeight() : g_Config.m_GfxScreenHeight;
	...
	Settings.m_Width = (g_Config.m_ClVideoWidth > 0 ? g_Config.m_ClVideoWidth : ScreenWidth) & ~1;
	Settings.m_Height = (g_Config.m_ClVideoHeight > 0 ? g_Config.m_ClVideoHeight : ScreenHeight) & ~1;
...
{
	return m_VideoSessionId.IsValid() ? &DemoSource(m_VideoSessionId).m_DemoPlayer : nullptr;
}
...
void CClient::SaveReplay(const int Length, const char *pFilename)
{
	...
	else
	{
		...
		const int EndTick = GameTick(SeatSessionId(g_Config.m_ClDummy));
...
const char *CClient::DemoPlayer_Play(CSessionId SessionId, const char *pFilename, int StorageType, bool Focus)
{
	...
	if(const char *pError = LoadDemo(SessionId, pFilename, StorageType))
	{
...
void CClient::Con_DemoSeek(IConsole::IResult *pResult, void *pUserData)
{
	...
	const IDemoPlayer::CInfo *pInfo = pSelf->DemoPlayer().BaseInfo();
	const float Current = (pInfo->m_CurrentTick - pInfo->m_FirstTick) / static_cast<float>(SERVER_TICK_SPEED);
	if(!pSelf->DemoPlayer().SeekTime(pResult->GetFloat(0) - Current))
...
void CClient::StopRenderTrace()
{
	if(!m_RenderTrace.Enabled())
		return;
	m_RenderTrace.Stop(Graphics()->GpuRenderZoneNames());
...
void CClient::RegisterCommands()
{
	...
	m_pConsole->Register("dbg_dump_sessions", "", CFGFLAG_CLIENT, Con_DbgDumpSessions, this, "Print the ticks of every session and connection");
...
#endif
{
	...
	if(VideoExport.m_Export)
	{
		...
		if(pStorage->FileExists(aVideoPath, IStorage::TYPE_SAVE_OR_ABSOLUTE))
		{
			log_error("videorecorder", "Output file '%s' already exists.", aVideoPath);
			PerformAllCleanup();
			return -1;
		}
		pClient->ConfigureCommandLineVideoExport(VideoExport.m_aDemoPath, aVideoPath, CCommandLineVideoExport::Settings());
...
{
	return m_pNetworkSessionSource->m_ServerCapabilities.m_SyncWeaponInput ? g_Config.m_ClPredictionMargin : 10;
}
