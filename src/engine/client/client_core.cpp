...
int *CClientCore::GetInput(CSessionId SessionId, int Conn, int Tick) const
{
	int Best = -1;
...
int CClientCore::GetPredictionTime(CSessionId SessionId, int Conn)
{
...
int CClientCore::GetPredictionTick(CSessionId SessionId, int Conn)
{
...
void CClientCore::GetSmoothTick(CSessionId SessionId, int Conn, int64_t Now, int *pSmoothTick, float *pSmoothIntraTick, float MixAmount)
{
...
const void *CClientCore::SnapFindItem(CSessionId SessionId, int Conn, int SnapId, int Type, int Id) const
{
...
int CClientCore::SnapNumItems(CSessionId SessionId, int Conn, int SnapId) const
{
	dbg_assert(SnapId >= 0 && SnapId < NUM_SNAPSHOT_TYPES, "invalid SnapId");
	if(!Connection(SessionId, Conn).m_apSnapshots[SnapId])
		return 0;
...
void CClientCore::OnDemoSnapshot(CSessionId SessionId, void *pData, int Size)
{
...
void CClientCore::OnDemoMessage(CSessionId SessionId, void *pData, int Size)
{
	CUnpacker Unpacker;
...
void CClientCore::UpdateDemoIntraTimers(CSessionId SessionId)
{
	// update timers
	CDemoSessionSource &Source = DemoSource(SessionId);
...
bool CClientCore::UpdateDemoPlayer(CSessionId SessionId)
{
