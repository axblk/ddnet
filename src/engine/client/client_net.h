...
#if defined(CONF_PLATFORM_IOS)
	/**
	 * Called after the sockets were recreated because the system had closed
	 * them, for whatever a program has told the old ones that the new ones do
	 * not know yet: the stun servers come from the DDNet info, which a program
	 * without a server browser never loads.
	 */
	virtual void OnSocketsRecreated() {}
#endif
	/**
