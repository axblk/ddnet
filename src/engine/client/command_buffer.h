...
struct SGpuTiming
{
	...
	std::array<uint64_t, IGraphics::MAX_GPU_RENDER_ZONES> m_aRenderZoneNanoseconds{};
...
struct SGpuTimingShared
{
	...
	std::array<std::atomic<uint64_t>, IGraphics::MAX_GPU_RENDER_ZONES> m_aRenderZoneNanoseconds{};
	...
	void Publish(uint64_t TimeNanoseconds, const std::array<uint64_t, IGraphics::MAX_GPU_RENDER_ZONES> &aRenderZoneNanoseconds = {}, uint32_t RenderZoneMask = 0)
	{
...
// The programs a draw can name. Texture, blend and clip variants are part of
// SState.
...
// What a program expects of the draw that uses it.
...
public:
	...
	// Commands are a linked list, so they do not need to be contiguous and the
	// arena grows in chunks instead of dropping a frame that does not fit.
	...
	struct SSubmissionInfo
	{
		...
		// Only a presented frame may be replaced by a newer one in the mailbox.
	...
	public:
		...
		unsigned m_Cmd;
		SCommand *m_pNext;
		// Signalled once the buffer has been run. A backend that leaves the work to
		// the device clears this and signals the completion itself.
	...
	{
		return AllocChunked(m_DataBuffer, m_vExtraDataBuffers, m_ExtraDataBufferIndex, WantedSize, alignof(std::max_align_t));
	}
	...
	bool AddCommandUnsafe(const T &Command)
	{
		...
		// The budget limits what queues behind a payload, not one payload's size.
		...
		T *pCmd = (T *)AllocChunked(m_CmdBuffer, m_vExtraCmdBuffers, m_ExtraCmdBufferIndex, sizeof(T), alignof(T));
		if(!pCmd)
			return false;
		*pCmd = Command;
...
private:
	static void *AllocChunked(CBuffer &First, std::vector<std::unique_ptr<CBuffer>> &vExtra, size_t &ExtraIndex, unsigned WantedSize, unsigned Alignment)
	{
		if(void *pData = First.Alloc(WantedSize, Alignment))
			return pData;
		while(ExtraIndex < vExtra.size())
		{
			if(void *pData = vExtra[ExtraIndex]->Alloc(WantedSize, Alignment))
				return pData;
			ExtraIndex++;
		}
		vExtra.emplace_back(std::make_unique<CBuffer>(std::max<unsigned>(First.Size(), WantedSize + Alignment)));
		return vExtra.back()->Alloc(WantedSize, Alignment);
	}
...
[[nodiscard]] inline const char *IndexedDrawInconsistency(const CCommandBuffer::SCommand_DrawIndexed &Cmd)
{
	...
	// Vulkan and WebGPU bake the vertex stride into the pipeline, so an
	// untextured draw cannot read a buffer that carries texture coordinates.
