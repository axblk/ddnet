...
struct SGraphicsBackendInit
{
	...
	bool m_VSync = false;
	int m_FsaaSamples = 0;
	// The OpenGL context version asked for. The other backends pick their API
	// version themselves.
...
public:
	...
	// Moves the commands into the backend's queue, waiting for room. The buffer
	// is empty afterwards.
	virtual bool RunBufferQueued(CCommandBuffer *pBuffer) = 0;
...
// Whether the backend at the configured version draws with programs.
bool IsModernGraphicsApi(EBackendType BackendType);

// The backend a name stands for, AUTO for a name that is unknown or not
// compiled in, and back.
...
// The renderers the compiled-in backends offer, from their own constants
// rather than a context. OpenGL versions the driver is blocked for are left
// out.
