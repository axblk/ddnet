...
void CGraphics_Threaded::RenderQuadLayer(CBufferHandle VertexBuffer, EVertexLayout Layout, SQuadRenderInfo *pQuadInfo, size_t QuadNum, int QuadOffset, bool Grouped)
{
	...
	// A draw the consistency check rejects has already been reported, once per
	// distinct reason. Reporting it again here, every frame and as an
	// allocation failure, says the wrong thing in a very loud way.
...
void CGraphics_Threaded::AdjustViewport(bool SendViewportChangeToBackend)
{
	...
	// than one. A surface of the window system is never bigger than a display
	// and needs no such limit. Before a backend has said, there is nothing to
	// go by and the frame is left as it was asked for.
