...
bool CRenderCommandQueue::CanEnqueueReliableLocked(const CCommandBuffer *pBuffer) const
{
	...
	// A buffer whose payload alone exceeds the budget can never share the queue with
	// anything else. Rejecting it outright would make the upload fail forever, so it
	// is admitted as soon as it can run on its own.
	if(pBuffer->m_ExternalDataSize > ExternalDataLimit)
		return m_ReliableExternalDataInFlight == 0;
...
void CRenderCommandQueue::WaitForIdle()
{
	std::unique_lock Lock(m_Mutex);
#if defined(CONF_PLATFORM_MACOS)
	// While the window is resized or moved, the render thread can be waiting
	// for the main thread's run loop. Pump it while waiting, or the two
	// threads wait for each other.
	while(!m_Condition.wait_for(Lock, std::chrono::milliseconds(1), [this] { return m_BuffersInFlight == 0; }))
	{
		Lock.unlock();
