...
bool CRenderTrace::Stop(std::span<const std::string> GpuZoneNames)
{
	if(!m_Enabled)
		return false;
	m_Enabled = false;
	const bool Saved = Save(GpuZoneNames);
	if(Saved)
...
{
	// A full trace stops rather than overwriting its start.
	return m_Enabled && (NowNanoseconds() >= m_StopNanoseconds || m_vFrames.size() >= MAX_FRAMES || m_vEvents.size() >= MAX_EVENTS);
}
...
void CRenderTrace::RecordEvent(const char *pName, IGraphics::CGpuRenderZone Zone, uint64_t StartNanoseconds, uint64_t DurationNanoseconds, uint64_t Generation)
{
...
uint32_t CRenderTrace::NameId(const char *pName, IGraphics::CGpuRenderZone Zone)
{
	const auto It = std::ranges::find(m_vNames, pName);
	if(It != m_vNames.end())
	{
		const size_t Index = static_cast<size_t>(std::distance(m_vNames.begin(), It));
		// The first caller to name a zone keeps it.
		if(!m_vNameZones[Index].IsValid())
...
bool CRenderTrace::Save(std::span<const std::string> GpuZoneNames) const
{
	...
	Writer.BeginArray();
	for(IGraphics::CGpuRenderZone Zone : m_vNameZones)
		Writer.WriteIntValue(Zone.Index());
	Writer.EndArray();
	Writer.WriteAttribute("gpu_zone_names");
	Writer.BeginArray();
	for(const std::string &Name : GpuZoneNames)
		Writer.WriteStrValue(Name.c_str());
	Writer.EndArray();

	Writer.WriteAttribute("frames");
	Writer.BeginArray();
	for(const CFrame &Frame : m_vFrames)
	{
		Writer.BeginObject();
		...
		Writer.BeginArray();
		for(size_t Zone = 0; Zone < GpuZoneNames.size(); ++Zone)
			Writer.WriteInt64Value(static_cast<int64_t>(Frame.m_Render.m_aGpuRenderZoneNanoseconds[Zone]));
		Writer.EndArray();
...
void CRenderTrace::Clear()
{
	...
	std::vector<IGraphics::CGpuRenderZone>().swap(m_vNameZones);
...
CRenderTraceScope::CRenderTraceScope(CRenderTrace *pTrace, const char *pName, IGraphics::CGpuRenderZone Zone)
{
