...
int CClient::TranslateSysMsg(int *pMsgId, bool System, CUnpacker *pUnpacker, CPacker *pPacker, const NETADDR *pPeerAddress, bool *pIsExMsg)
{
	*pIsExMsg = false;
	if(!System)
		return -1;
	...
	{
		CTranslationContext &TranslationContext = m_pNetworkSessionSource->m_TranslationContext;
	...
	else if(*pMsgId == protocol7::NETMSG_SERVERINFO)
	{
		...
		*pMsgId = -1;
		CServerInfo &ServerInfo = m_pNetworkSessionSource->m_ServerInfo;
