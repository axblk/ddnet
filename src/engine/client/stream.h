...
public:
	/**
	 * @param InitialSize Number of streams the storage starts with.
	 * @param Neutral Value of a stream that has not been written yet. A stream
	 * that appears later has to start out the same way as the first two, and for
	 * a client id that is -1, not 0.
	 */
	explicit CStreamStorage(size_t InitialSize = 0, T Neutral = T{}) :
		m_vValues(InitialSize, Neutral),
		m_Neutral(Neutral)
	{
	}
	...
	decltype(auto) operator[](int Index)
	{
		...
		if(static_cast<size_t>(Index) >= m_vValues.size())
			m_vValues.resize(Index + 1, m_Neutral);
