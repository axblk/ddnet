...
private:
	...
	// Pts is where the frame belongs on the timeline, in frames since the
	// first one that was written. It is not the number of frames written: a
	// frame that never came back leaves a gap there, and a gap is what keeps
	// the picture that follows it where the sound expects it.
	void SubmitVideoFrame(CImageInfo Image, int64_t Pts);
	// The export throws its first rendered frame away, so the frame that opens
	// the video is the second one the recorder counted.
	static constexpr uint64_t FIRST_WRITTEN_VIDEO_FRAME = 2;
	...
	bool m_Started;
	// The '.partial' file exists from here on, so it has to be cleaned up even
	// if starting the export failed after it was created.
	bool m_TemporaryFileCreated = false;
	bool m_Stopped;
	bool m_Recording;
	...
	// The conversion pass can fail per frame and hand back plain colour instead.
	bool m_ReportedYuvFallback = false;
	...
	class CPendingVideoWrite
	{
	public:
		size_t m_FrameIndex = NO_PENDING_WRITE;
		int64_t m_Pts = 0;
	};
	std::array<CPendingVideoWrite, 2 * MAX_VIDEO_THREADS + 2> m_aPendingVideoWrites;
