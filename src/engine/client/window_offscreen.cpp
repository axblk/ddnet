...
public:
	IGraphicsBackend *Open(bool Hidden) override
	{
		...
		// Draws nothing; the size only keeps the interface's layout sane.
		const EBackendType BackendOverride = BACKEND_TYPE_NULL;
		g_Config.m_GfxScreenWidth = 800;
		g_Config.m_GfxScreenHeight = 600;
#else
		const EBackendType BackendOverride = GraphicsBackendOverrideFromEnvironment();
#endif
		IGraphicsBackend *pBackend = CreateOffscreenGraphicsBackend(BackendOverride);
		if(pBackend == nullptr)
		{
	...
	// Nothing about this window ever changes on its own.
