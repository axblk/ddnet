...
class CGraphicsWindow_SDL : public IEngineGraphicsWindow, public IPresentationSurface
{
	...
	bool SetWindowScreen(int Index, bool MoveToCenter);
	bool UpdateDisplayMode(int Index);
	bool ResizeWindow(int w, int h, int RefreshRate);
	void ReadDisplayCutout();
	int CollectVideoModes(CVideoMode *pModes, int MaxModes, float HiDPIScale, int ScreenId);
	void GetCurrentVideoMode(CVideoMode &CurMode, int ScreenId);
...
// Called on the render thread as well.
...
static bool GetWebGpuNativeWindow(SDL_Window *pWindow, SWebGpuNativeWindow &NativeWindow)
{
	...
	case SDL_SYSWM_ANDROID:
		...
		NativeWindow.m_Type = SWebGpuNativeWindow::EType::ANDROID_WINDOW;
...
{
	EBackendType RetBackendType = m_BackendOverride;
	if(RetBackendType == BACKEND_TYPE_AUTO)
	{
		RetBackendType = BackendTypeFromName(g_Config.m_GfxBackend);
		if(RetBackendType == BACKEND_TYPE_AUTO || RetBackendType == BACKEND_TYPE_NULL)
...
int CGraphicsWindow_SDL::CollectVideoModes(CVideoMode *pModes, int MaxModes, float HiDPIScale, int ScreenId)
{
	...
	for(int i = 0; i < NumModes; i++)
	{
		...
		if(Mode.w > m_DesktopSize.x || Mode.h > m_DesktopSize.y)
			continue;
	...
	return NumModesInserted;
}

void CGraphicsWindow_SDL::GetCurrentVideoMode(CVideoMode &CurMode, int ScreenId)
{
	...
	else
	{
		...
		else
		{
			...
			DpMode.w = Width / m_HiDPIScale;
			DpMode.h = Height / m_HiDPIScale;
		}
	}
	DisplayToVideoMode(&CurMode, &DpMode, m_HiDPIScale, DpMode.refresh_rate);
}
...
bool CGraphicsWindow_SDL::UpdateDisplayMode(int Index)
{
...
// Picks the backend for the next attempt. A backend that failed to start the
// time before falls back to OpenGL, and the config remembers that, so a
// system without it does not pay for the attempt on every start.
void CGraphicsWindow_SDL::ChooseBackend()
{
	const EBackendType OldBackendType = m_BackendType;
	...
	if(OldBackendType == BACKEND_TYPE_WEBGPU && m_BackendType == BACKEND_TYPE_WEBGPU)
	{
		...
		g_Config.m_GfxGLMinor = 0;
		g_Config.m_GfxGLPatch = 0;
		// 3.0 is the version the driver blocklist arms itself on.
		g_Config.m_GfxDriverIsBlocked = 0;
	}
	if(OldBackendType != BACKEND_TYPE_AUTO && m_BackendType == BACKEND_TYPE_VULKAN)
	{
		str_copy(g_Config.m_GfxBackend, "OpenGL");
		g_Config.m_GfxGLMajor = 3;
		g_Config.m_GfxGLMinor = 0;
...
// Everything SDL has to know before it creates a window with an OpenGL context.
void CGraphicsWindow_SDL::SetGlAttributes()
{
	...
	else if(IsModernGraphicsApi(m_BackendType))
	{
		// The forward compatible flag holds the program backend to core from
		// 3.0 on, where profiles only exist from 3.2.
	...
	SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS, g_Config.m_GfxFsaaSamples ? 1 : 0);
	SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES, g_Config.m_GfxFsaaSamples);
}

bool CGraphicsWindow_SDL::CreateWebGpuNativeWindow()
{
#if !defined(CONF_BACKEND_WEBGPU)
	return false;
	...
	return m_WebGpuNativeWindow.m_pWindow != nullptr;
#else
	return GetWebGpuNativeWindow(m_pWindow, m_WebGpuNativeWindow);
#endif
}
...
int CGraphicsWindow_SDL::OpenWindow(SGraphicsBackendInit &BackendInit)
{
	...
	if(m_BackendType == BACKEND_TYPE_WEBGPU)
		...
		log_info("gfx", "Created %s %d.%d context", BackendTypeName(m_BackendType), g_Config.m_GfxGLMajor, g_Config.m_GfxGLMinor);
	...
	const int ModesCount = CollectVideoModes(aModes, std::size(aModes), 1, g_Config.m_GfxScreen);
	...
	if(m_BackendType == BACKEND_TYPE_WEBGPU && !CreateWebGpuNativeWindow())
	{
	...
	{
		DrawableSize(m_Surface.m_DrawableWidth, m_Surface.m_DrawableHeight);
	}
	else
	{
...
void CGraphicsWindow_SDL::SetWindowParams(int FullscreenMode, bool IsBorderless)
{
	...
	CVideoMode CurMode;
	GetCurrentVideoMode(CurMode, g_Config.m_GfxScreen);
...
void CGraphicsWindow_SDL::OnMoved(int x, int y)
{
	...
	if(!UpdateDisplayMode(SDL_GetWindowDisplayIndex(m_pWindow)))
		return;
...
int CGraphicsWindow_SDL::GetVideoModes(CVideoMode *pModes, int MaxModes, int ScreenIndex)
{
	...
	return CollectVideoModes(pModes, MaxModes, m_HiDPIScale, ScreenIndex);
}
