...
int CGraphicsWindow_SDL::OpenWindow(SGraphicsBackendInit &BackendInit)
{
	...
	if(OldBackendType == BACKEND_TYPE_WEBGPU && m_BackendType == BACKEND_TYPE_WEBGPU)
	{
		...
		g_Config.m_GfxGLMinor = 0;
		g_Config.m_GfxGLPatch = 0;
		// 3.0 is the version the driver blocklist arms itself on, so a stale
		// blocked state would be handed to OpenGL unchecked - and it hides 3.0
		// and 3.3 from the renderer list. The Vulkan fallback below clears it
		// for the same reason.
