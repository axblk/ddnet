...
protected:
	// The vertex buffer that Begin/Draw/End fill. The calls between a Begin and
	// an End are plain member functions; only the flush is virtual.
	enum class EDrawing
	{
		NONE,
		QUADS,
	...
	void Rotate(const vec2 &Center, TName *pPoints, int NumPoints)
	{
		...
		{
			const float dx = pPoints[i].m_Pos.x - Center.x;
			const float dy = pPoints[i].m_Pos.y - Center.y;
			pPoints[i].m_Pos.x = dx * c - dy * s + Center.x;
			pPoints[i].m_Pos.y = dx * s + dy * c + Center.y;
		}
	}
...
public:
	...
	// CreateTexture with TEXTURE_USAGE_COLOR_TARGET returns an invalid handle
	// on backends without render targets.
	...
	// Every buffer the client draws from has one of these shapes.
	...
	// A layered texture is derived by splitting the uploaded 2D image into a
	// grid. The backend decides between an array and a volume.
	...
	struct CTextureDesc
	{
		...
		bool IsValid() const
		{
			...
			if(m_Width == 0 || m_Height == 0 || m_Usage == 0 || (m_Usage & ~AllUsages) != 0)
				return false;
			if(m_Format != ETextureFormat::RGBA8_UNORM && m_Mipmaps != ETextureMipmaps::NONE)
				return false;
	...
	static const SVertexLayoutDesc &VertexLayout(EVertexLayout Layout)
	{
		...
		static constexpr std::array<SVertexLayoutDesc, (size_t)EVertexLayout::COUNT> s_aLayouts = {{
			// TILE
		...
		static_assert(std::all_of(s_aLayouts.begin(), s_aLayouts.end(), [](const SVertexLayoutDesc &Desc) {
			return std::all_of(Desc.m_aAttributes.begin(), Desc.m_aAttributes.begin() + Desc.m_AttributeCount, [&](const CVertexAttributeDesc &Attribute) {
				return Attribute.m_Offset + Attribute.m_ComponentCount * (Attribute.m_Type == EType::FLOAT32 ? sizeof(float) : 1) <= Desc.m_Stride;
			});
		}),
			"Vertex attribute does not fit its stride");
	...
	// Takes a malloc() allocation and frees it once consumed. On failure (an
	// invalid handle or false) the caller still owns and frees it.
...
// What the client and the window need from the graphics beyond drawing.
