...
class CRegister : public IRegister
{
	...
	static const SProtocolInfo PROTOCOLS[];
...
const CRegister::SProtocolInfo CRegister::PROTOCOLS[] = {
	{"tw0.6/ipv6", "register/6/ipv6", "tw-0.6+udp://", ETransport::UDP, false, IPRESOLVE::V6},
	{"tw0.6/ipv4", "register/6/ipv4", "tw-0.6+udp://", ETransport::UDP, false, IPRESOLVE::V4},
	{"tw0.7/ipv6", "register/7/ipv6", "tw-0.7+udp://", ETransport::UDP, true, IPRESOLVE::V6},
	{"tw0.7/ipv4", "register/7/ipv4", "tw-0.7+udp://", ETransport::UDP, true, IPRESOLVE::V4},
	{"ddnet+quic/ipv6", "register/quic/6/ipv6", "ddnet+quic://", ETransport::QUIC, false, IPRESOLVE::V6},
	{"ddnet+quic/ipv4", "register/quic/6/ipv4", "ddnet+quic://", ETransport::QUIC, false, IPRESOLVE::V4},
	{"tw0.7+quic/ipv6", "register/quic/7/ipv6", "tw-0.7+quic://", ETransport::QUIC, true, IPRESOLVE::V6},
	{"tw0.7+quic/ipv4", "register/quic/7/ipv4", "tw-0.7+quic://", ETransport::QUIC, true, IPRESOLVE::V4},
	{"ddnet+wt/ipv6", "register/wt/6/ipv6", "ddnet+wt://", ETransport::WEBTRANSPORT, false, IPRESOLVE::V6},
	{"ddnet+wt/ipv4", "register/wt/6/ipv4", "ddnet+wt://", ETransport::WEBTRANSPORT, false, IPRESOLVE::V4},
	{"tw0.7+wt/ipv6", "register/wt/7/ipv6", "tw-0.7+wt://", ETransport::WEBTRANSPORT, true, IPRESOLVE::V6},
	{"tw0.7+wt/ipv4", "register/wt/7/ipv4", "tw-0.7+wt://", ETransport::WEBTRANSPORT, true, IPRESOLVE::V4},
	{"ddnet+ws/ipv6", "register/ws/6/ipv6", "ddnet+ws://", ETransport::WEBSOCKET, false, IPRESOLVE::V6},
	{"ddnet+ws/ipv4", "register/ws/6/ipv4", "ddnet+ws://", ETransport::WEBSOCKET, false, IPRESOLVE::V4},
};

const char *CRegister::ProtocolToScheme(int Protocol) const
{
	if(PROTOCOLS[Protocol].m_Transport == ETransport::WEBSOCKET && m_Transports.m_WebsocketTls)
	{
		return "ddnet+wss://";
	}
	return PROTOCOLS[Protocol].m_pScheme;
}

const char *CRegister::ProtocolToString(int Protocol)
{
	return PROTOCOLS[Protocol].m_pName;
}
...
bool CRegister::ProtocolFromString(int *pResult, const char *pString)
{
	...
	{
		if(str_comp(pString, PROTOCOLS[Protocol].m_pName) == 0)
		{
			*pResult = Protocol;
			return false;
		}
...
{
	return PROTOCOLS[Protocol].m_pSystem;
}

IPRESOLVE CRegister::ProtocolToIpresolve(int Protocol)
{
	return PROTOCOLS[Protocol].m_Ipresolve;
}
...
void CRegister::CProtocol::CheckChallengeStatus()
{
	...
	if(m_pShared->m_LatestResponseIndex == m_pShared->m_NumTotalRequests - 1)
	{
		...
		case STATUS_NEEDCHALLENGE:
			if(m_NewChallengeToken || PROTOCOLS[m_Protocol].m_Transport == ETransport::QUIC || PROTOCOLS[m_Protocol].m_Transport == ETransport::WEBTRANSPORT)
			{
...
void CRegister::CProtocol::CJob::Run()
{
	...
	if(Status == STATUS_ERROR)
	{
		...
		const bool UnsupportedResponse = PROTOCOLS[m_Protocol].m_Transport != ETransport::UDP && (m_pRegister->StatusCode() == 400 || m_pRegister->StatusCode() == 501);
	...
	}
	{
		...
		if(Status == m_pShared->m_LatestResponseStatus && Status == STATUS_NEEDCHALLENGE && PROTOCOLS[m_Protocol].m_Transport == ETransport::UDP)
		{
...
{
	static_assert(std::size(PROTOCOLS) == NUM_PROTOCOLS);
