...
MACRO_CONFIG_INT(EdShowIngameEntities, ed_show_ingame_entities, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Show how weapons, shields, snowflakes and flags appear ingame")
...
MACRO_CONFIG_INT(GfxWholeWindow, gfx_whole_window, 0, 0, 1, CFGFLAG_CLIENT, "Draw into the whole window however tall it is, instead of keeping the drawn area within five by four")
...
MACRO_CONFIG_INT(ClDummySplitScreen, cl_dummy_split_screen, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_INSENSITIVE, "Show the player and the dummy side by side, and a demo beside them when picture in picture is off")
MACRO_CONFIG_INT(ClPictureInPicture, cl_picture_in_picture, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE | CFGFLAG_INSENSITIVE, "Show the server while a demo has focus, or the demo while the server has focus, in a corner of the screen")
MACRO_CONFIG_INT(ClPictureInPictureSound, cl_picture_in_picture_sound, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE | CFGFLAG_INSENSITIVE, "Play the sound of the server or demo without focus instead of the one with focus")
MACRO_CONFIG_INT(ClPictureInPictureSize, cl_picture_in_picture_size, 30, 10, 50, CFGFLAG_CLIENT | CFGFLAG_SAVE | CFGFLAG_INSENSITIVE, "Width of the picture in picture, in percent of the screen width")
...
MACRO_CONFIG_STR(SvGametype, sv_gametype, 32, "ddnet", CFGFLAG_SERVER, "Registered game mode ID")
...
MACRO_CONFIG_INT(ClQuic, cl_quic, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Offer QUIC where a server announces it")
MACRO_CONFIG_INT(ClConnectProtocol, cl_connect_protocol, -1, -1, 3, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Transport to connect with, as last picked next to the address: -1 = the best the server offers, 0 = legacy, 1 = QUIC, 2 = websocket, 3 = WebTransport")
MACRO_CONFIG_INT(ClConnectAddressFamily, cl_connect_address_family, 1, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Address family to connect with, as last picked next to the address: 0 = IPv4 only, 1 = IPv6 where the server has it and IPv4 where it does not")
MACRO_CONFIG_STR(ClQuicCert, cl_quic_cert, IO_MAX_PATH_LENGTH, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "Pinned DER certificate for native QUIC direct connections")
MACRO_CONFIG_STR(ClQuicServerName, cl_quic_server_name, 128, "", CFGFLAG_CLIENT | CFGFLAG_SAVE, "TLS server name override for native QUIC direct connections")
MACRO_CONFIG_INT(ClWebtransport, cl_webtransport, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Enable advertised WebTransport in browser clients")
...
MACRO_CONFIG_STR(SvWebsocketCert, sv_websocket_cert, IO_MAX_PATH_LENGTH, "", CFGFLAG_SERVER | CFGFLAG_NONTEEHISTORIC, "Path to the TLS certificate chain used for websocket connections, enables wss instead of ws (requires sv_websocket_key). Prefer an ECDSA certificate, its handshake is much cheaper than RSA")
MACRO_CONFIG_STR(SvWebsocketKey, sv_websocket_key, IO_MAX_PATH_LENGTH, "", CFGFLAG_SERVER | CFGFLAG_NONTEEHISTORIC, "Path to the TLS private key used for websocket connections (requires sv_websocket_cert)")
MACRO_CONFIG_STR(SvWebsocketOrigin, sv_websocket_origin, 256, "", CFGFLAG_SERVER | CFGFLAG_NONTEEHISTORIC, "Exact browser Origin accepted for websocket connections, empty accepts all origins")
#endif
MACRO_CONFIG_STR(SvTlsCert, sv_tls_cert, IO_MAX_PATH_LENGTH, "", CFGFLAG_SERVER, "PEM certificate chain or DER certificate shared by QUIC and WebTransport (restart required)")
MACRO_CONFIG_STR(SvTlsCertNext, sv_tls_cert_next, IO_MAX_PATH_LENGTH, "", CFGFLAG_SERVER, "Next PEM or DER certificate advertised during certificate rotation (restart required)")
...
MACRO_CONFIG_STR(SvQuicIdentityKey, sv_quic_identity_key, IO_MAX_PATH_LENGTH, "quic_identity.pk8", CFGFLAG_SERVER | CFGFLAG_NONTEEHISTORIC, "Persistent Ed25519 server identity key, generated when missing (restart required)")
MACRO_CONFIG_INT(SvQuicResumeGraceMs, sv_quic_resume_grace_ms, 10000, 1000, 60000, CFGFLAG_SERVER, "Time to retain an authenticated QUIC player slot after transport loss in milliseconds")
MACRO_CONFIG_INT(SvWebtransport, sv_webtransport, 1, 0, 1, CFGFLAG_SERVER, "Enable WebTransport on the server UDP port (restart required)")
MACRO_CONFIG_STR(SvWebtransportHostname, sv_webtransport_hostname, 256, "", CFGFLAG_SERVER, "Public DNS name or bracketed IP advertised for WebTransport; optional in hash mode (restart required)")
MACRO_CONFIG_STR(SvWebtransportCertificateMode, sv_webtransport_certificate_mode, 16, "hash", CFGFLAG_SERVER, "WebTransport certificate verification mode: hash or webpki (restart required)")
...
MACRO_CONFIG_INT(SvMaxPreInputsPerTick, sv_max_preinputs_per_tick, 8, 0, 1000, CFGFLAG_SERVER, "Maximum number of inputs per tick and client that are sent on to the other clients as preinput (0 for no limit)")
...
MACRO_CONFIG_STR(SvRegisterHostname, sv_register_hostname, 256, "", CFGFLAG_SERVER, "Public DNS hostname registered for QUIC and WebTransport; supported masters retain it instead of the observed IP (restart required)")
...
MACRO_CONFIG_INT(SvServerInfoRepliesPerSecond, sv_server_info_replies_per_second, 500, 0, 1000000, CFGFLAG_SERVER, "Maximum number of server info responses of any size that are sent out per second, the requesting address is not verified in 0.6 (0 for no limit)")
...
MACRO_CONFIG_INT(SvVanConnRepliesPerSecond, sv_van_conn_replies_per_second, 200, 0, 1000000, CFGFLAG_SERVER, "Maximum number of answers to vanilla connections per second, both antispoof handshakes and refusals, they are answered to addresses that are not verified yet (0 for no limit)")
...
MACRO_CONFIG_INT(SvConnDecompressPerSecond, sv_conn_decompress_per_second, 250, 0, 1000000, CFGFLAG_SERVER, "Maximum number of compressed 0.6 packets per connection that are decompressed per second, their security token is inside the compressed payload and cannot be checked beforehand (0 for no limit)")
MACRO_CONFIG_STR(SvEbpfKey, sv_ebpf_key, IO_MAX_PATH_LENGTH, "/run/ddnet-xdp/key", CFGFLAG_SERVER | CFGFLAG_NONTEEHISTORIC, "Key file written by the ddnet-xdp filter service, derives security tokens and QUIC connection IDs the filter can verify (empty to disable)")
...
MACRO_CONFIG_INT(ConnResendRequestsPerSecond, conn_resend_requests_per_second, 10, 0, 1000, CFGFLAG_CLIENT | CFGFLAG_SERVER, "Maximum number of resend requests of a peer that are answered per second, one answer resends everything that is not acked yet (0 for no limit)")
MACRO_CONFIG_INT(ClCompressUpstream, cl_compress_upstream, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SAVE, "Compress packets sent to 0.6 servers. Off leaves the security token readable on the wire so a server side flood filter can tell this connection apart from a spoofed one, at the cost of a little upstream bandwidth")
