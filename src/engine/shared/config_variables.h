...
MACRO_CONFIG_INT(SvWebtransport, sv_webtransport, 1, 0, 1, CFGFLAG_SERVER, "Accept WebTransport clients on the server port, needs sv_quic (restart required)")
...
MACRO_CONFIG_STR(SvRegisterHostname, sv_register_hostname, 256, "", CFGFLAG_SERVER, "Public DNS hostname registered for QUIC and WebTransport; supported masters keep it instead of the observed IP. With sv_tls_cert, browsers check the WebTransport certificate against it instead of by hash (restart required)")
...
MACRO_CONFIG_INT(SvDdnetConnections, sv_ddnet_connections, 1, 0, 1, CFGFLAG_SERVER, "Enable connections from DDNet 0.6 clients")
...
MACRO_CONFIG_INT(DbgTlsKeyLog, dbg_tls_key_log, 0, 0, 1, CFGFLAG_CLIENT | CFGFLAG_SERVER, "Write the TLS session keys of the QUIC, WebTransport and WebSocket transports to the file named by the environment variable SSLKEYLOGFILE, for Wireshark (restart required)")
