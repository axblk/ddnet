...
namespace
{
	...
	// The layout of the filter service's `struct key_file`.
...
bool CEbpfKey::Load(const char *pPath)
{
	...
	if(!File)
	{
		log_info("ebpf", "no key at '%s', the filter will not arm this port", pPath);
		return false;
	}
	...
	const CKeyFileEpoch &Epoch = KeyFile.m_aEpochs[KeyFile.m_Current];
	m_aMaterial[0] = (unsigned char)KeyFile.m_Current;
	for(int Byte = 0; Byte < 8; Byte++)
	{
		m_aMaterial[1 + Byte] = (unsigned char)(Epoch.m_K0 >> (8 * Byte));
		m_aMaterial[9 + Byte] = (unsigned char)(Epoch.m_K1 >> (8 * Byte));
	}
...
bool CEbpfKey::Reload(const char *pPath)
{
	...
	if(!Load(pPath))
	{
		// A file that is briefly unreadable must not take the server out of
		// the filter's sight; the material is left as it was.
		m_Loaded = WasLoaded;
		return false;
	}
	return !WasLoaded || mem_comp(aPrevious, m_aMaterial, sizeof(m_aMaterial)) != 0;
}
