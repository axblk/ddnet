 ...
 *
 * The classic network code derives the tokens itself with `Token()`; the Rust
 * networking library takes the material through `ddnet_net_set_filter_key` and
 * derives the tokens and the QUIC connection IDs on its own.
 */
class CEbpfKey
{
public:
