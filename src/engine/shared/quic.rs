...
pub mod ffi {
    ...
    extern "Rust" {
        ...
        fn quic_server_identity_fingerprint(endpoint: &QuicEndpoint) -> Vec<u8>;
...
impl ServerCertificatePin {
    fn new(pin: ffi::QuicPin, fingerprints: &[u8]) -> Result<Self, String> {
        let identity = |expected_fingerprint| {
        ...
        match pin {
            ffi::QuicPin::Tofu => Ok(identity(None)),
            ...
            ffi::QuicPin::Identity => {
                Ok(identity(Some(fingerprints.try_into().map_err(|_| {
                    "server identity fingerprint must contain 32 bytes"
                })?)))
            }
...
pub fn quic_server_start(
    ...
    let identity_proof = if raw_quic {
        ...
        let signing_key = Ed25519KeyPair::from_pkcs8(&key)
            .map_err(|error| format!("invalid server identity key: {error}"))?;
        Some(ServerIdentityProof {
            public_key: signing_key.public_key().as_ref().try_into().unwrap(),
            signing_key: Arc::new(signing_key),
            certificate_sha256: ring::digest::digest(
                &SHA256,
                certificate_chain(certificate_file)?[0].as_ref(),
            )
            .as_ref()
            .try_into()
            .unwrap(),
    ...
    let (config, certificate_resolver) = server_config(
        raw_quic,
        webtransport,
        certificate_file.to_vec(),
        private_key_file.to_vec(),
    )?;
...
/// Returns the fingerprint of the identity key of a server serving raw QUIC.
pub fn quic_server_identity_fingerprint(endpoint: &QuicEndpoint) -> Vec<u8> {
    endpoint.inner.lock().map_or_else(
        |_| Vec::new(),
        |endpoint| {
            endpoint
                .server_identity_public_key()
                .map_or_else(Vec::new, |public_key| {
                    ring::digest::digest(&SHA256, &public_key).as_ref().to_vec()
                })
        },
    )
}
...
pub fn quic_client_start(
    ...
    let (config, verification) = client_config(ServerCertificatePin::new(pin, fingerprints)?)?;
...
fn hello_payload(
    ...
    let payload = game_wire::encode_hello_with(
        extra_capabilities | sixup,
        game_wire::MAX_DATAGRAM_SIZE,
        nonce,
        resume_binding,
    )
...
mod tests {
    ...
    fn webtransport_only_does_not_require_server_identity() {
        ...
        .unwrap();
        assert!(quic_server_identity_fingerprint(&endpoint).is_empty());
    }
}
