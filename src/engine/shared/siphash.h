// SipHash-2-4, the derivation a packet filter in front of the server and the
// server share for security tokens and QUIC connection IDs. The filter
// compiles the same source for the BPF target (`src/xdp/siphash.h` on the
// filter's branch), so the two cannot drift apart; the tests pin the vectors
// of the specification and the tokens derived from them.
//
// Reference: Jean-Philippe Aumasson and Daniel J. Bernstein, "SipHash: a fast
// short-input PRF" (2012).
//
#ifndef ENGINE_SHARED_SIPHASH_H
#define ENGINE_SHARED_SIPHASH_H
