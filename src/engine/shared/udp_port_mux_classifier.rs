...
pub fn classify(
    ...
    // A QUIC short header carries a connection ID that has to verify, the
    // connectionless prefixes carry nothing that does. Asking QUIC first
    // therefore costs the legacy protocols nothing, while asking the prefixes
    // first loses a QUIC packet whenever header protection happens to produce a
    // first byte of 'x' in front of a connection ID that starts with 'e'.
...
mod tests {
    ...
    fn classifies_legacy_quic_collisions_without_dual_delivery() {
        ...
        // Header protection can put an 'x' in front of a connection ID that
        // starts with an 'e'. That is a connectionless prefix, and the packet
        // still belongs to QUIC.
        const XE_CID: [u8; QUIC_CID_LEN] = [b'e', 2, 3, 4, 5, 6, 7, 8];
        let mut xe_short = vec![b'x'];
        xe_short.extend_from_slice(&XE_CID);
        xe_short.push(0);
        assert_eq!(
            super::classify(&xe_short, || false, XE_CID.len(), |cid| cid == XE_CID),
            DatagramRoute::Quic
        );
