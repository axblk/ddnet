...
void CChat::Reset()
{
	...
	for(auto &aLastSoundPlayed : m_aaLastSoundPlayed)
		std::fill(std::begin(aLastSoundPlayed), std::end(aLastSoundPlayed), 0);
}
...
bool CChat::OnInput(const IInput::CEvent &Event)
{
	if(m_Mode == MODE_NONE)
		return false;
	CGameSessionContext &InputSession = GameClient()->SessionContext(m_InputBinding.m_SessionId);
	const CSessionPresentation &Presentation = GameClient()->SessionPresentation(m_InputBinding.m_SessionId);
	const std::vector<CSessionChatState::CCommand> &vServerCommands = InputSession.m_Chat.SortedCommands();
...
void CChat::HandleMessage(CGameSessionContext &Session, const CGameState &State, int64_t Now, bool SuppressEvents, bool IsDemoPlayback, bool ApplicationEffects, int MsgType, void *pRawMsg)
{
	...
	else if(MsgType == NETMSGTYPE_SV_COMMANDINFO)
	{
		...
		Session.m_Chat.BeginCommandInfo();
		Session.m_Chat.RegisterCommand(pMsg->m_pName, pMsg->m_pArgsFormat, pMsg->m_pHelpText);
	}
	else if(MsgType == NETMSGTYPE_SV_COMMANDINFOREMOVE)
	{
		CNetMsg_Sv_CommandInfoRemove *pMsg = (CNetMsg_Sv_CommandInfoRemove *)pRawMsg;
		Session.m_Chat.UnregisterCommand(pMsg->m_pName);
	}
}
...
void CChat::AddLine(CGameSessionContext &Session, const CGameState &State, int64_t Now, bool IsDemoPlayback, bool ApplicationEffects, int ClientId, int Team, const char *pLine)
{
	...
	if(ClientId >= 0)
	{
		...
		AuthorLocal = std::any_of(Session.GameStates().begin(), Session.GameStates().end(), [ClientId](const CGameState &OtherState) { return OtherState.LocalClientId() == ClientId; });
	...
	if(ClientId >= 0 && !AuthorLocal)
	{
		...
		{
			const int LocalId = SessionState.LocalClientId();
			if(!in_range(LocalId, MAX_CLIENTS - 1))
				continue;
			const CGameState::CClientIdentityState &LocalIdentity = SessionState.ClientIdentity(LocalId);
			char aLocalName[64];
	...
	const uint64_t PreviousLastId = Session.m_Chat.LastId();
	const CSessionChatState::CLine &StoredLine = Session.m_Chat.Add(NewLine);
	...
	bool OfflineAudio;
	const bool AudioEffects = GameClient()->AudioForSession(Session.Id(), OfflineAudio);
	...
	if(ClientId == SERVER_MSG)
	{
		if(AudioEffects && Now - pLastSoundPlayed[CHAT_SERVER] >= time_freq() * 3 / 10)
		{
			...
			{
				GameClient()->m_Sounds.Play(CSounds::CHN_GUI, SOUND_CHAT_SERVER, 1.0f, OfflineAudio);
	...
	else if(Highlighted && !IsDemoPlayback)
	{
		if(Now - pLastSoundPlayed[CHAT_HIGHLIGHT] >= time_freq() * 3 / 10)
		{
			...
			{
				GameClient()->m_Sounds.Play(CSounds::CHN_GUI, SOUND_CHAT_HIGHLIGHT, 1.0f, OfflineAudio);
	...
	else if(Team != TEAM_WHISPER_SEND)
	{
		if(AudioEffects && Now - pLastSoundPlayed[CHAT_CLIENT] >= time_freq() * 3 / 10)
		{
			...
			if(PlaySound)
			{
				GameClient()->m_Sounds.Play(CSounds::CHN_GUI, SOUND_CHAT_CLIENT, 1.0f, OfflineAudio);
...
{
	return m_Mode != MODE_NONE && Context.m_View.Binding() == m_InputBinding;
}

bool CChat::ShowMatches(const CRenderContext &Context) const
{
	return m_Show && Context.m_View.Binding() == m_ShowBinding;
}
...
void CChat::OnPrepareLines(const CRenderContext &Context, float y)
{
	...
	const CSessionChatState &Chat = Context.m_Session.m_Chat;
...
void CChat::RenderApplicationOverlay(const CRenderContext &Context)
{
	...
	const CGameSessionContext *pSession = GameClient()->FindSessionContext(m_InputBinding.m_SessionId);
	if(pSession == nullptr || m_Input.GetString()[0] != '/' || m_Input.GetString()[1] == '\0')
		return;
	for(const CSessionChatState::CCommand &Command : pSession->m_Chat.Commands())
	{
...
void CChat::SendChatQueued(const char *pLine)
{
	...
	bool AddEntry = false;
	CGameSessionContext *pSession = GameClient()->FindSessionContext(m_InputBinding.m_SessionId);
	if(pSession == nullptr)
		return;
