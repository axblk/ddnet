...
void CGameConsole::CInstance::ExecuteLine(const char *pLine)
{
	...
	else
	{
		...
		else
		{
			...
			{
				m_pGameConsole->Client()->RconAuth(m_pGameConsole->Client()->ActiveConnection(), m_aUser, pLine);
				m_UserGot = false;
			}
		}
	}
...
void CGameConsole::OnRenderApplicationOverlay()
{
...

	{
		...
		m_VersionText.Render(TextRender(), vec2(ButtonBar.x + ButtonBar.w - m_VersionText.Width() - 10.0f, FONT_SIZE / 2.0f), TextRender()->DefaultTextColor());
	}
}
