...
{
	State.m_DamageIndicators.Create(Pos, Dir, OwnerClientId, Alpha, -random_angle());
}
...
void CDamageInd::Update(const CPresentationContext &Context)
{
	if(!Context.m_Time.m_IsGameActive)
		return;
	Context.m_State.m_DamageIndicators.Advance(Context.m_Time.m_PresentationTime, Context.m_Time.m_PresentationTimeFrequency, Context.m_Time.m_AnimationPlaybackSpeed);
}
...
void CDamageInd::OnRender(const CRenderContext &Context)
{
	...
	const CGameState::CDamageIndicatorState &State = Context.m_State.m_DamageIndicators;
