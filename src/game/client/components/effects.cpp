...
{
	if(!State.m_EffectClock.m_Add50hz)
		return;
...
{
	if(!State.m_EffectClock.m_Add5hz)
		return;
...
{
	if(!State.m_EffectClock.m_Add50hz && TimePassed < 0.001f)
		return;
...
{
	if(!State.m_EffectClock.m_Add100hz && TimePassed < 0.001f)
		return;
...
void CEffects::Explosion(CGameState &State, const CCollision &Collision, vec2 Pos, float Alpha)
{
	// add the explosion
	CParticle p;
	p.SetDefault();
...
void CEffects::Update(const CPresentationContext &Context)
{
	...
	State.m_EffectClock.Update(Time.m_PresentationTime, Time.m_PresentationTimeFrequency, Time.m_AnimationPlaybackSpeed);
	if(Time.m_IsGameActive)
		State.m_SceneClock.Update(Time.m_PresentationTime, Time.m_PresentationTimeFrequency, Time.m_AnimationPlaybackSpeed, Time.m_GameTickTime, Time.m_PredIntraGameTick);
}
