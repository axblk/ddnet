	...
	// render small score hud
	{
		...
		if(TeamPlay && pGameData != nullptr)
		{
			...
			for(int t = 0; t < 2; t++)
			{
				...
				{
					const int FlagDropTick = State.m_Runtime.m_aFlagDropTick[t];
			...
			for(int t = 0; t < 2; ++t)
			{
				if(apPlayerInfo[t])
				{
					...
					else if(Context.m_State.CoreGameInfo().m_TimeScore)
					{
						...
						if(Context.m_State.m_Runtime.m_ReceivedDDNetPlayerFinishTimes && FinishTimeSeconds != FinishTime::UNSET && FinishTimeSeconds != FinishTime::NOT_FINISHED_MILLIS)
						{
...
void CHud::RenderCursor(const CRenderContext &Context)
{
	...
	{
		// Render spec cursor
		const CGameView::CSpectatorCursorState &Cursor = Context.m_View.m_SpectatorCursor;
...
void CHud::RenderPlayerState(const CRenderContext &Context, const int ClientId)
{
	...
	if(g_Config.m_ClShowhudJumpsIndicator)
	{
		...
		{
			const bool Grounded = Context.m_Session.m_MapContext.Collision()->IsOnGround(vec2(pPlayer->m_X, pPlayer->m_Y), CCharacterCore::PhysicalSize());
...
void CHud::RenderSpectatorHud(const CRenderContext &Context)
{
	...
	// draw the text
	char aBuf[128];
	if(Context.m_View.m_MultiView.m_Active)
	{
	...
	if(ShowHud)
	{
		...
		else if(Context.m_View.IsSpectating())
		{
			...
			if(pSpectatedClient != nullptr &&
				...
				(!Context.m_View.m_MultiView.m_Active || Context.m_View.m_MultiView.m_ShowHud) &&
...
{
	const CGameState::CRaceMessageState &RaceMessages = Context.m_State.m_RaceMessages;
...
void CHud::RenderRecord(const CRenderContext &Context)
{
	...
	if(Context.m_State.m_Runtime.m_ReceivedDDNetPlayerFinishTimes && PlayerTimeSeconds != FinishTime::UNSET)
	{
	...
	{
		const float PlayerRecord = Context.m_State.m_Runtime.m_PlayerRecord;
