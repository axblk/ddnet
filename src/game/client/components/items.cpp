...
{
	const CGameState::CSceneClockState &SceneClock = State.m_SceneClock;
...
void CItems::RenderProjectile(const CRenderContext &Context, const CProjectileData *pCurrent, int ItemId, const CScreenRect &ScreenRect, bool PredictProjectiles)
{
	...
	if(CurWeapon == WEAPON_GRENADE)
	{
		Graphics()->QuadsSetRotation(Context.m_State.m_SceneClock.m_AnimationTime * pi * 2 * 2 + ItemId);
	}
	else
	{
...
{
	const CGameState::CSceneClockState &SceneClock = Context.m_State.m_SceneClock;
...
void CItems::RenderLaser(const CRenderContext &Context, const CLaserData *pCurrent, bool IsPredicted)
{
	...
	int TuneZone = 0;
	if((Type == LASERTYPE_RIFLE || Type == LASERTYPE_SHOTGUN) && !Context.m_Time.m_IsDemoPlayback && Context.m_State.m_GameWorld.m_WorldConfig.m_UseTuneZones)
	{
		const CCollision *pCollision = Context.m_Session.m_MapContext.Collision();
...
CItems::CSwitchFilter::CSwitchFilter(const CGameState &State, const CGameTickInfo &Time) :
	m_State(State)
{
	const int LocalClientId = State.LocalClientId();
	const CNetObj_DDNetCharacter *pLocalExtended = State.ExtendedCharacter(LocalClientId);
	m_IsSuper = pLocalExtended != nullptr && (pLocalExtended->m_Flags & CHARACTERFLAG_SUPER) != 0;
	if(State.m_Runtime.m_SwitchStateTeam >= 0)
		m_Team = State.m_Runtime.m_SwitchStateTeam;
	else if(!in_range(LocalClientId, MAX_CLIENTS - 1))
		m_Team = 0;
	else if(State.HasSpectatorInfo() && in_range(State.SpectatorInfo().m_SpectatorId, MAX_CLIENTS - 1))
		m_Team = State.Teams().Team(State.SpectatorInfo().m_SpectatorId);
	else
		m_Team = State.Teams().Team(LocalClientId);
	m_Ticks = Time.m_GameTick % Time.m_GameTickSpeed;
}

bool CItems::CSwitchFilter::IsInactive(int Number) const
{
	const std::vector<SSwitchers> &vSwitchers = m_State.m_GameWorld.m_Core.m_vSwitchers;
	return !m_IsSuper && Number > 0 && Number < (int)vSwitchers.size() && !vSwitchers[Number].m_aStatus[m_Team];
}

bool CItems::CSwitchFilter::HidesProjectile(int Number, bool Explosive) const
{
	return IsInactive(Number) && (Explosive ? (m_Ticks % 6) < 2 : (m_Ticks % 20) < 2);
}

bool CItems::CSwitchFilter::HidesPickup(int Number) const
{
	return IsInactive(Number) && (m_Ticks % 22) < 4;
}

bool CItems::PredictsProjectiles(const CGameState &State, const CGameTickInfo &Time)
{
	const int LocalClientId = State.LocalClientId();
	const bool WorldPaused = State.HasGameInfo() && (State.GameInfo().m_GameStateFlags & (GAMESTATEFLAG_GAMEOVER | GAMESTATEFLAG_PAUSED)) != 0;
	return g_Config.m_ClPredict && g_Config.m_ClAntiPing && g_Config.m_ClAntiPingGrenade && !WorldPaused && !Time.m_IsDemoPlayback &&
	       !State.HasSpectatorInfo() && in_range(LocalClientId, MAX_CLIENTS - 1) && State.Client(LocalClientId).m_HasCharacter;
}

bool CItems::UsePredictedItems(const CGameState &State, const CGameTickInfo &Time)
{
	return PredictsProjectiles(State, Time) && g_Config.m_ClAntiPingWeapons && g_Config.m_ClAntiPingGunfire;
}
...
void CItems::UpdatePresentation(const CPresentationContext &Context)
{
	...
	const int LocalClientId = State.LocalClientId();
	const CSwitchFilter Filter(State, Context.m_Time);
	const bool PredictProjectiles = PredictsProjectiles(State, Context.m_Time);
	const bool UsePredicted = UsePredictedItems(State, Context.m_Time);
	CGameWorld &GameWorld = State.m_GameWorld;
	CGameWorld &PrevPredictedWorld = State.m_PrevPredictedWorld;
	...
	auto AddProjectileEffects = [&](const CProjectileData &Data) {
		...
		if(!GetProjectileRenderInfo(State, Context.m_Time, PredictProjectiles, IsOtherTeam, &Data, Context.m_vVisibleWorldRects, vec2(TileSize, TileSize), CurWeapon, Pos, Vel, Alpha))
			return;
	...
	if(UsePredicted)
	{
		...
		{
			if(Filter.HidesProjectile(pProj->m_Number, pProj->m_Explosive))
				continue;
		...
		{
			if(pPickup->m_Layer == LAYER_SWITCH && Filter.HidesPickup(pPickup->m_Number))
				continue;
	...
	for(const CGameState::CEntitySnapshot &Entity : State.Entities())
	{
		...
		if(Entity.m_Type == NETOBJTYPE_PROJECTILE || Entity.m_Type == NETOBJTYPE_DDRACEPROJECTILE || Entity.m_Type == NETOBJTYPE_DDNETPROJECTILE)
		{
			...
			if(Filter.HidesProjectile(Data.m_SwitchNumber, Data.m_Explosive))
				continue;
		...
		else if(Entity.m_Type == NETOBJTYPE_PICKUP || Entity.m_Type == NETOBJTYPE_DDNETPICKUP)
		{
			...
			if(!Context.IsVisible(Data.m_Pos, vec2(1.75f * TileSize, 0.75f * TileSize)))
				continue;
			if(Filter.HidesPickup(Data.m_SwitchNumber))
				continue;
			if(UsePredicted)
			{
...
void CItems::OnRender(const CRenderContext &Context)
{
	...
	const CSwitchFilter Filter(State, Context.m_Time);
	const bool BlinkingGun = (Filter.m_Ticks % 22) < 4;
	const bool BlinkingDragger = (Filter.m_Ticks % 22) < 4;
	const bool BlinkingLight = (Filter.m_Ticks % 6) < 2;
	const int DraggerStartTick = std::max((Context.m_Time.m_GameTick / 7) * 7, Context.m_Time.m_GameTick - 4);
	const int GunStartTick = (Context.m_Time.m_GameTick / 7) * 7;

	const bool UsePredicted = UsePredictedItems(State, Context.m_Time);
	const bool PredictProjectiles = PredictsProjectiles(State, Context.m_Time);
	const CGameWorld &GameWorld = State.m_GameWorld;
	const CGameWorld &PrevPredictedWorld = State.m_PrevPredictedWorld;
	...
	for(const CGameState::CEntitySnapshot &Entity : State.Entities())
	{
		...
		else if(ItemType == NETOBJTYPE_LASER || ItemType == NETOBJTYPE_DDNETLASER)
		{
			...
			if(!IsLaserInside(Data))
				continue;
			const bool Inactive = Filter.IsInactive(Data.m_SwitchNumber);
			...
			{
				if(Data.m_Predict && (Inactive || Filter.IsSuper()))
				{
...
void CItems::OnInit()
{
	...
	RenderTools()->GetSpriteScale(SPRITE_PICKUP_FREEZE, ScaleX, ScaleY);
	Graphics()->QuadsSetSubset(0, 0, 1, 1);
	m_PickupFreezeOffset = RenderTools()->QuadContainerAddSprite(m_ItemsQuadContainerIndex, 64.f * ScaleX, 64.f * ScaleY);
