...
{
	// Items of switches that are off for the team of the local or spectated player blink.
	class CSwitchFilter
	{
	...
	public:
		...
		CSwitchFilter(const CGameState &State, const CGameTickInfo &Time);
		bool IsInactive(int Number) const;
		bool HidesProjectile(int Number, bool Explosive) const;
		bool HidesPickup(int Number) const;
		bool IsSuper() const { return m_IsSuper; }
	};
	static bool PredictsProjectiles(const CGameState &State, const CGameTickInfo &Time);
	static bool UsePredictedItems(const CGameState &State, const CGameTickInfo &Time);
