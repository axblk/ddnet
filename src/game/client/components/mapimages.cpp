...
void CMapRenderImages::Load(class CLayers *pLayers, IMap *pMap, bool Sixup)
{
	...
	for(int i = 0; i < m_Count; i++)
	{
		...
		if(pImg->m_External)
		{
			...
			m_vImageLoads.push_back({i, LoadFlag, m_pAssetLoader->LoadImageFile(Storage(), aPath, IStorage::TYPE_ALL, m_AssetOwnerId, m_Generation)});
		}
		else
		{
			...
			m_vImageLoads.push_back({i, LoadFlag, m_pAssetLoader->LoadImageRawData(std::move(RawData), pImg->m_Width, pImg->m_Height, CImageInfo::FORMAT_RGBA, aTexName, m_AssetOwnerId, m_Generation)});
		}
