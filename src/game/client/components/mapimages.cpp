...
void CMapImages::FinishEntitiesLoads()
{
	...
	m_SpeedupArrowResource.FinishTexture(Graphics(), m_SpeedupArrowTexture, Graphics()->TextureLoadFlags() | IGraphics::TEXLOAD_NO_2D_TEXTURE);
}
...
void CMapImages::OnMapLoadImpl(class CLayers *pLayers, IMap *pMap)
{
	...
	for(int i = 0; i < m_Count; i++)
	{
		...
		if(pImg->m_External)
		{
			...
			m_vExternalImageLoads.push_back({i, LoadFlag, GameClient()->AssetLoader().LoadImageFile(Storage(), aPath, IStorage::TYPE_ALL)});
		}
		else
		{
