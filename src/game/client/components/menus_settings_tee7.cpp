...
void CMenus::RenderSettingsTee7(CUIRect MainView)
{
	...
	for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
	{
		GameClient()->m_Skins7.FindSkinPart(Part, apSkinPartsPtr[Part], false)->ApplyTo(OwnSkinInfo.m_Sixup);
		GameClient()->m_Skins7.ApplyColorTo(OwnSkinInfo.m_Sixup, aUCCVars[Part], aColorVars[Part], Part);
	}
	...
	for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
	{
		GameClient()->m_Skins7.FindSkinPart(Part, apSkinPartsPtr[Part], false)->ApplyTo(TeamSkinInfo.m_Sixup);
		TeamSkinInfo.m_Sixup.m_aUseCustomColors[Part] = aUCCVars[Part];
	}

	for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
	{
		TeamSkinInfo.m_Sixup.m_aColors[Part] = GameClient()->m_Skins7.GetTeamColor(aUCCVars[Part], aColorVars[Part], TEAM_RED, Part);
	}
	RenderTools()->RenderTee(CAnimState::GetIdle(), &TeamSkinInfo, 0, vec2(1, 0), RedTeamSkinPreview.Center() + OffsetToMid);

	for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
	{
		TeamSkinInfo.m_Sixup.m_aColors[Part] = GameClient()->m_Skins7.GetTeamColor(aUCCVars[Part], aColorVars[Part], TEAM_BLUE, Part);
	}
	...
	if(Ui()->DoEditBox_Search(&s_SkinFilterInput, &QuickSearch, 14.0f, !Ui()->IsPopupOpen() && !Frontend()->m_GameConsole.IsActive()))
	{
	...
	if(DoButton_Menu(&s_SkinRefreshButton, FontIcon::ARROW_ROTATE_RIGHT, 0, &RefreshButton) ||
		(!Ui()->IsPopupOpen() && !Frontend()->m_GameConsole.IsActive() && (Input()->KeyPress(KEY_F5) || (Input()->ModifierIsPressed() && Input()->KeyPress(KEY_R)))))
	{
...
void CMenus::RenderSkinSelection7(CUIRect MainView)
{
	...
	for(const CSkins7::CSkin *pSkin : s_vpSkinList)
	{
		...
		for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
		{
			pSkin->m_apParts[Part]->ApplyTo(Info.m_Sixup);
			GameClient()->m_Skins7.ApplyColorTo(Info.m_Sixup, pSkin->m_aUseCustomColors[Part], pSkin->m_aPartColors[Part], Part);
		}
		Info.m_Size = 50.0f;
...
void CMenus::RenderSkinPartSelection7(CUIRect MainView)
{
	...
	for(const CSkins7::CSkinPart *pPart : s_avpList[m_TeePartSelected])
	{
		...
		for(int Part = 0; Part < protocol7::NUM_SKINPARTS; Part++)
		{
			...
			pPreviewPart->ApplyTo(Info.m_Sixup);
			GameClient()->m_Skins7.ApplyColorTo(Info.m_Sixup, *CSkins7::ms_apUCCVariables[(int)m_Dummy][Part], *CSkins7::ms_apColorVariables[(int)m_Dummy][Part], Part);
		}
		Info.m_Size = 50.0f;
