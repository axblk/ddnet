...
{
	if(State.m_Runtime.m_GameOver || State.m_Runtime.m_GamePaused || GameClient()->IsDemoPlaybackPaused())
		return;
	State.m_Particles.Add(Group, Particle, TimePassed);
}
...
void CParticles::Update(const CPresentationContext &Context)
{
	...
	CGameState::CParticleSystemState &State = Context.m_State.m_Particles;
	...
	if(State.m_TimeInitialized && Time.m_PresentationTime >= State.m_LastRenderTime && Time.m_PresentationTimeFrequency > 0)
	{
		...
		UpdatePhysics(State, *Context.m_Session.m_MapContext.Collision(), TimePassed);
	}
...
{
	const CGameState::CParticleSystemState &State = Context.m_State.m_Particles;
