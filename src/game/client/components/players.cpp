	...
	int ClientId)
{
	...
	const CPhysicsRules Rules = GameClient()->PredictedPhysicsRules(Context.m_Session, GameState);
	...
	if(!Context.m_Session.ServerCapAnyPlayerFlag())
	{
		...
		{
			if(ClientId == SessionState.LocalClientId())
			{
				Aim = SessionState.Input().m_ShowHookColl;
				break;
			}
		}
	}
	...
	for(const CGameState &SessionState : Context.m_Session.GameStates())
	{
		if(SessionState.LocalClientId() == ClientId)
		{
			pSessionLocalState = &SessionState;
			break;
		}
	}
	...
	CPlayerRenderState &State)
{
	...
	const CGameState::CSceneClockState &SceneClock = GameState.m_SceneClock;
	...
	State.m_InAir = !Session.m_MapContext.Collision()->CheckPoint(State.m_Player.m_X, State.m_Player.m_Y + 16);
	...
	float Intra)
{
	...
	if(g_Config.m_ClShowEmotes && !GameClient()->SessionPresentation(Context.m_Session.Id()).EmoticonIgnored(ClientId) && Emoticon.m_StartTick != -1)
	{
		...
		if(0 <= SinceStart && FromEnd > 0)
		{
			...
			float WiggleAngle = std::sin(2 * pi * Wiggle);
