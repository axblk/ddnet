...
void CScoreboard::RenderSpectators(const CRenderContext &Context, CUIRect Spectators)
{
	...
	if(pClientsByName == nullptr)
		return;
	const int Corners = m_MouseUnlocked ? IGraphics::CORNER_ALL : IGraphics::CORNER_T;
	GameClient()->m_Menus.DrawSurface(Spectators, ColorRGBA(0.0f, 0.0f, 0.0f, 0.5f), Corners, 7.5f);
...
void CScoreboard::RenderScoreboard(const CRenderContext &Context, CUIRect Scoreboard, int Team, int CountStart, int CountEnd, CScoreboardRenderState &State, int NumPlayersForSize)
{
	...
	const int MaxTeamSize = SentMaxTeamSize != 0 ? SentMaxTeamSize : Context.m_Session.MapContext().GameConfig().Values().m_SvMaxTeamSize;
