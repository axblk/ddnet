...
bool CSkins::LoadSkinData(const char *pName, CImageInfo &Info, CSkinLoadData &Data) const
{
...
std::function<bool(CImageInfo &)> CSkins::SkinPostprocess(CSkinContainer *pSkinContainer)
{
	...
	pSkinContainer->m_pLoadData = pData;
	return [this, pData, Name = std::string(pSkinContainer->Name())](CImageInfo &Info) {
		return LoadSkinData(Name.c_str(), Info, *pData);
	};
}
...
void CSkins::StartLocalSkinLoad(CSkinContainer *pSkinContainer)
{
	...
	pSkinContainer->m_LoadResource = GameClient()->AssetLoader().LoadImageFile(Storage(), aPath, pSkinContainer->StorageType(), SkinPostprocess(pSkinContainer));
}
...
void CSkins::UpdateStartLoading(CSkinLoadingStats &Stats)
{
	...
	for(auto &[_, pSkinContainer] : m_Skins)
	{
		...
		case CSkinContainer::EType::DOWNLOAD:
			StartDownload(pSkinContainer.get(), false);
			break;
		default:
...
void CSkins::UpdateFinishLoading(CSkinLoadingStats &Stats, std::chrono::nanoseconds StartTime, std::chrono::nanoseconds MaxTime)
{
	for(auto &[_, pSkinContainer] : m_Skins)
	{
		...
		if(Resource.IsReady())
		{
			pSkinContainer->m_pLoadData->m_Info = Resource.TakeImage();
			const bool Success = LoadSkinFinish(pSkinContainer.get(), *pSkinContainer->m_pLoadData);
			Resource.Reset();
		...
		{
			log_error("skins", "Failed to load skin '%s' from '%s'", pSkinContainer->Name(), Resource.Path());
			pSkinContainer->SetState(CSkinContainer::EState::ERROR);
			Stats.m_NumError++;
		}
		Resource.Reset();
