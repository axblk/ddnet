...
void CStatboard::RenderBoard(const CRenderContext &Context, const CStoredMatch &Stored)
{
	...
	GameClient()->m_Menus.RenderBackdropRegion({X, Y, Width, Height}, IGraphics::CORNER_ALL, 17.0f);
	RenderTools()->DrawRect(X, Y, Width, Height, ColorRGBA(0.0f, 0.0f, 0.0f, 0.6f), IGraphics::CORNER_ALL, 17.0f);
	...
	for(size_t Team = 0; Team < Report.m_vTeams.size(); ++Team)
	{
		RenderTools()->DrawRect(ContentX, ContentY, ContentWidth, 60.0f, TeamColor(Team).WithAlpha(static_cast<int>(Team) == Leader ? 0.45f : 0.18f), IGraphics::CORNER_ALL, 6.0f);
	...
	if(NumAspects > 1)
	{
		...
		for(size_t Index = 0; Index < NumAspects; ++Index)
		{
			...
			if(Candidate == Aspect)
				RenderTools()->DrawRect(TabX - 6.0f, ContentY - 2.0f, Text.Width() + 12.0f, 28.0f, ColorRGBA(0.30f, 0.62f, 1.0f, 0.35f), IGraphics::CORNER_ALL, 4.0f);
...
float CStatboard::RenderTable(const CRenderContext &Context, const CStoredMatch &Stored, float X, float Y, float Width)
{
	...
	for(size_t Index = 0; Index < m_NumColumns; ++Index)
	{
		...
		if(Column.m_aLabel[0] == '\0' && Column.m_Weapon >= 0 && Column.m_Weapon < NUM_WEAPONS)
		{
			...
			float ScaleX, ScaleY;
			RenderTools()->GetSpriteScale(g_pData->m_Weapons.m_aId[Weapon].m_pSpriteBody, ScaleX, ScaleY);
			...
			RenderTools()->DrawSprite(ColumnX + Column.m_Width / 2.0f, Y + 12.0f, Size * ScaleX, Size * ScaleY);
			Graphics()->QuadsEnd();
		}
		else
		{
	...
	for(const CRow &Row : Rows())
	{
		...
		if(Row.m_pParticipant != nullptr && Row.m_TeamIndex != TeamIndex && Row.m_TeamIndex >= 0 && Row.m_TeamIndex < static_cast<int>(Report.m_vTeams.size()))
		{
			...
			RenderTools()->DrawRect(X, Y + 2.0f, Width, 22.0f, TeamColor(TeamIndex).WithAlpha(0.35f), IGraphics::CORNER_ALL, 4.0f);
