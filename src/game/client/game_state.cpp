...
namespace
{
	...
	{
		// The padding between the members of a struct is never written by an
		// assignment, so it keeps whatever the memory held before. Hashing a
		// padded struct as bytes mixes that in, and two states that say the same
		// thing get different digests - on one platform and not on the next. A
		// digested struct therefore has to be free of gaps, which is checked
		// here rather than found in a Windows build weeks later.
		static_assert(!std::is_class_v<T> || std::has_unique_object_representations_v<T>,
			"a digested struct must not have padding between its members");
		DigestBytes(Digest, &Value, sizeof(Value));
	}

	/**
	 * The core game info member by member.
	 *
	 * This one mixes flags and numbers and so has gaps between its members,
	 * which the whole-object digest above rightly refuses. A member reaches the
	 * digest by being listed here, in the order the class declares them.
	 */
	void DigestGameInfo(uint64_t &Digest, const CGameInfo &Info)
	{
		DigestValue(Digest, Info.m_FlagStartsRace);
		DigestValue(Digest, Info.m_TimeScore);
		DigestValue(Digest, Info.m_UnlimitedAmmo);
		DigestValue(Digest, Info.m_DDRaceRecordMessage);
		DigestValue(Digest, Info.m_RaceRecordMessage);
		DigestValue(Digest, Info.m_RaceSounds);
		DigestValue(Digest, Info.m_AllowEyeWheel);
		DigestValue(Digest, Info.m_AllowHookColl);
		DigestValue(Digest, Info.m_AllowZoom);
		DigestValue(Digest, Info.m_BugDDRaceGhost);
		DigestValue(Digest, Info.m_BugDDRaceInput);
		DigestValue(Digest, Info.m_BugFNGLaserRange);
		DigestValue(Digest, Info.m_BugVanillaBounce);
		DigestValue(Digest, Info.m_PredictFNG);
		DigestValue(Digest, Info.m_PredictDDRace);
		DigestValue(Digest, Info.m_PredictDDRaceTiles);
		DigestValue(Digest, Info.m_PredictVanilla);
		DigestValue(Digest, Info.m_EntitiesDDNet);
		DigestValue(Digest, Info.m_EntitiesDDRace);
		DigestValue(Digest, Info.m_EntitiesRace);
		DigestValue(Digest, Info.m_EntitiesFNG);
		DigestValue(Digest, Info.m_EntitiesVanilla);
		DigestValue(Digest, Info.m_EntitiesBW);
		DigestValue(Digest, Info.m_EntitiesFDDrace);
		DigestValue(Digest, Info.m_Race);
		DigestValue(Digest, Info.m_Pvp);
		DigestValue(Digest, Info.m_DontMaskEntities);
		DigestValue(Digest, Info.m_AllowXSkins);
		DigestValue(Digest, Info.m_HudHealthArmor);
		DigestValue(Digest, Info.m_HudAmmo);
		DigestValue(Digest, Info.m_HudDDRace);
		DigestValue(Digest, Info.m_NoWeakHookAndBounce);
		DigestValue(Digest, Info.m_NoSkinChangeForFrozen);
...
void CGameState::SetCoreGameInfo(const CGameInfo &GameInfo)
{
	...
	m_Teams.m_NumDDRaceTeams = GameInfo.m_NumDDRaceTeams;
...
uint64_t CGameState::SnapshotDigest() const
{
	...
	DigestGameInfo(Digest, m_CoreGameInfo);
