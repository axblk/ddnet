...
void CGameState::ApplySnapshot(const IClient &Client, CSessionId SessionId, int Conn)
{
	...
	ApplySnapshotData(Client.GameTick(SessionId, Conn), aClients, HasGameInfo ? &GameInfo : nullptr, std::move(vEntities));
	if(HasSpectatorInfo)
...
{
	PredictTo(Client.PredGameTick(SessionId, Conn), [&Client, SessionId, Conn](int Tick) {
		return reinterpret_cast<const CNetObj_PlayerInput *>(Client.GetInput(SessionId, Conn, Tick));
	});
}
