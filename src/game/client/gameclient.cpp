...
// Points a view at whatever its own state says should be watched. Every state
// builds its snap state now, so this is the answer the focused session used to
// get out of ProcessSnapshot, asked once per view instead of once per client.
void CGameClient::AimView(const CGameSessionContext &Session, const CGameState &State, CGameView &View) const
{
	const CGameState::CSnapState &Snap = State.Snap();
	View.SetSpectator(Snap.m_SpecInfo.m_Active, Snap.m_SpecInfo.m_SpectatorId);
	if(Client()->SessionType(Session.Id()) != ESessionSourceType::NETWORK)
	{
		// A demo is watched the way whoever opened it chose to watch it. A demo
		// being rendered to video is watched the way it was recorded, which is
		// what carries the zoom the server sent into the exported frames.
		View.SetSpectatorMode(Session.Id() == Client()->DemoSessionId() ? m_DemoSpecId : SPEC_FOLLOW);
	}
}

// Works out where the camera of one view should be looking, from the state that
// view is drawing. Only the view that takes input owns the client-wide local
// character position, everything else is read and written on the state itself.
...
{
	CGameView::CMultiViewState &MultiViewState = View.MultiView();
	CGameState::CSnapState &Snap = State.Snap();
...
void CGameClient::OnRender()
{
	...
	auto RenderRequestComponents = [&](std::span<const SRenderComponent> Components) {
		m_RenderScheduler.Run(
			...
			[this, &Components](const CRenderContext &Context, CRenderOutput &Output) {
				...
				{
					CRenderTraceScope TraceScope(m_pRenderTrace, pName, GpuZone);
					if(GpuZone != IGraphics::EGpuRenderZone::COUNT)
						Graphics()->GpuRenderZoneBegin(GpuZone);
					pComponent->OnRender(Context);
					if(GpuZone != IGraphics::EGpuRenderZone::COUNT)
	...
	if(!m_PreparedIsolatedVideoOutput)
	{
		...
		{
			CRenderTraceScope TraceScope(pTrace, pName, GpuZone);
			if(GpuZone != IGraphics::EGpuRenderZone::COUNT)
				Graphics()->GpuRenderZoneBegin(GpuZone);
		...
		{
			CRenderTraceScope TraceScope(pTrace, "ui/chat_application", IGraphics::EGpuRenderZone::CHAT);
	...
	if(!m_PreparedIsolatedVideoOutput)
	{
		...
		{
			CRenderTraceScope TraceScope(pTrace, "ui/touch_controls_application", IGraphics::EGpuRenderZone::TOUCH_CONTROLS);
	...
	if(!m_PreparedIsolatedVideoOutput)
	{
		ScreenOutput.BeginView(View.Viewport(), View.CameraPosition(), View.Zoom());
		{
			CRenderTraceScope TraceScope(pTrace, "ui/scoreboard_application", IGraphics::EGpuRenderZone::SCOREBOARD);
			Graphics()->GpuRenderZoneBegin(IGraphics::EGpuRenderZone::SCOREBOARD);
...
void CGameClient::PrepareScreenRender(bool VideoOutput)
{
	...
	// Every view is pointed at what its own state says should be watched before
	// the controllers run, because they read where the view is looking.
	for(CPreparedRenderEntry &Entry : m_vPreparedRenderEntries)
	...
	// Every view gets the same camera, driven by the state it is drawing. The one
	// that takes input is the only one that may move the mouse or run the
	// controls; the rest are told where to look and follow the same rules about
	// zoom, spectating and smoothing as it does.
	for(CPreparedRenderEntry &Entry : m_vPreparedRenderEntries)
	{
...
void CGameClient::OnRenderVideoPrepare(CSessionId SessionId, const CVideoExportSettings &Settings)
{
	...
	AimView(*pSession, *pState, *pView);
	m_Camera.BindTarget(*pSession, *pState, *pView, false, LocalTime);
	UpdatePositions(*pState, *pView, Entry.m_Time, LocalTime, false);
	m_Camera.UpdateCamera();
	m_Camera.UpdatePosition();
...
void CGameClient::OnMessage(CSessionId SessionId, int MsgId, CUnpacker *pUnpacker, CStreamId StreamId)
{
	...
	if(MsgId == NETMSGTYPE_SV_TEAMSSTATE || MsgId == NETMSGTYPE_SV_TEAMSSTATELEGACY)
	{
		...
		if(i <= VANILLA_MAX_CLIENTS)
			pMessageState->SetNumDDRaceTeams(VANILLA_MAX_CLIENTS + 1);
		if(AdditionalStream)
			return;
...
// Everything a game state needs in order to be drawn: who is playing, where they
// are, and which camera the server asked for. It runs for every state the client
// keeps - the connection being played, the dummy, a demo, a demo being rendered
// to video - because any of them can be on screen at the same time. What only
// the focused session has, the legacy client data and the messages sent back to
// the server, stays in ProcessSnapshot.
void CGameClient::BuildSnapState(CSessionId SessionId, int Conn)
{
	...
	// The names are kept on the game state as the numbers the server sent, so
	// they are decoded once here rather than inside the comparator.
...
void CGameClient::ProcessSnapshot(CSessionId SessionId, int Conn)
{
	...
	// The snapshot was already taken apart into the game state, so the legacy
	// client data is filled from there instead of walking the items a second
	// time. Only the focused session keeps it, which is why it cannot be built
	// where the snap state is.
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ++ClientId)
	{
		CClientData &Client = m_aClients[ClientId];
...
{
	// The player settings send their change to the server as they are read,
