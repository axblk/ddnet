...
public:
	...
	CMapContext &MapContext() const { return SessionContext().m_MapContext; }
	...
	class CLayers *Layers() const { return MapContext().Layers(); }
	CCollision *Collision() const { return MapContext().Collision(); }
	const CConfig *GameConfig() const { return &MapContext().GameConfig().Values(); }
	...
	void SendStartInfo7(int Conn);
	void SendSkinChange7(int Conn) const;
	// Returns true if the requested skin change got applied by the server
	bool GotWantedSkin7(int Conn) const;
	...
	CPhysicsRules PredictedPhysicsRules(const CGameSessionContext &Session, const CGameState &State) const;
