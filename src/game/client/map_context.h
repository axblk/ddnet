...
// same way. Sessions share one through a shared pointer, so it lives as long as
// the last of them.
class CMapData
{
...
class CMapContext
{
	...
	// Doors are stamped into it, the switch count is written from snapshots and
	// predicted lasers write tiles, so each session keeps its own.
...
public:
	...
	// Leaves the map to whoever else plays it and starts over with an empty one.
	void Unload();
