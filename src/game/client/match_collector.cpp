...
bool CSessionStatsState::FinalizeObservedMatch(const CObservedMatchMetadata &Metadata, const CGameState &State, int Tick)
{
	...
	Report.m_vParticipants.push_back({0, TeamId, aName, aClan, std::clamp<int64_t>(Stats.FirstJoinTick() - m_RoundStartTick, 0, DurationTicks)});
	...
	const auto AddMetric = [&](const char *pMetricId, int64_t Value) {
		...
		Report.m_vMetrics.push_back({EMatchSubjectKind::PARTICIPANT, 0, pMetricId, Value, pInfo ? pInfo->m_Aggregation : EMatchMetricAggregation::SUM});
	};
	AddMetric("score", LocalScore);
	AddMetric("kills", Stats.m_Frags);
	AddMetric("deaths", Stats.m_Deaths);
	AddMetric("suicides", Stats.m_Suicides);
	AddMetric("best_spree", Stats.m_BestSpree);
	if(Stats.m_FlagGrabs != 0)
		AddMetric("flag_grabs", Stats.m_FlagGrabs);
	if(Stats.m_FlagCaptures != 0)
		AddMetric("flag_captures", Stats.m_FlagCaptures);
	AddMetric("playtime_ticks", Stats.GetIngameTicks(Tick));
	for(int Weapon = 0; Weapon < NUM_WEAPONS; ++Weapon)
	{
		char aMetricId[32];
		if(Stats.m_aFragsWith[Weapon] != 0)
		{
			str_format(aMetricId, sizeof(aMetricId), "weapon_%d_kills", Weapon);
			AddMetric(aMetricId, Stats.m_aFragsWith[Weapon]);
		}
		if(Stats.m_aDeathsFrom[Weapon] != 0)
		{
			str_format(aMetricId, sizeof(aMetricId), "weapon_%d_deaths", Weapon);
			AddMetric(aMetricId, Stats.m_aDeathsFrom[Weapon]);
		}
	}
...
void CSessionStatsState::SetLatestServerMatch(CStoredMatch Match)
{
	...
	if(m_LiveStats.has_value() && m_LiveStats->m_Report.m_MatchId == Match.m_Report.m_MatchId)
		m_LiveStats.reset();
...
const CStoredMatch *CSessionStatsState::ObservedMatchReplacedBy(const CStoredMatch &ServerMatch) const
{
	const std::optional<CStoredMatch> &Observed = m_LatestMatch.has_value() && m_LatestMatch->m_Source == EMatchReportSource::CLIENT_OBSERVED ? m_LatestMatch : m_PreviousObservedMatch;
	if(!Observed.has_value() || Observed->m_Source != EMatchReportSource::CLIENT_OBSERVED || Observed->m_OriginId != ServerMatch.m_OriginId ||
		Observed->m_Report.m_MapSha256 != ServerMatch.m_Report.m_MapSha256 || Observed->m_Report.m_RoundStartTick != ServerMatch.m_Report.m_RoundStartTick)
		return nullptr;
	return &*Observed;
}

void CSessionStatsState::SetLiveStats(CStoredMatch Match, bool PersistOnDisconnect)
{
	m_LiveStats = std::move(Match);
	m_PersistLiveStats = PersistOnDisconnect;
}

const CStoredMatch *CSessionStatsState::LiveStatsToPersist() const
{
	// the final report of the round, if it came, says everything the snapshot says
	if(!m_LiveStats.has_value() || !m_PersistLiveStats || !IsCurrentServerMatch(m_LiveStats->m_Report))
		return nullptr;
	return &*m_LiveStats;
}
