...
class CSessionStatsState
{
	...
	std::optional<CStoredMatch> m_LiveStats;
	bool m_PersistLiveStats = false;
...
public:
	...
	bool FinalizeObservedMatch(const CObservedMatchMetadata &Metadata, const CGameState &State, int Tick);
	...
	// what the client observed of the round the server reports, nullptr if nothing
	const CStoredMatch *ObservedMatchReplacedBy(const CStoredMatch &ServerMatch) const;
	void SetLiveStats(CStoredMatch Match, bool PersistOnDisconnect);
	// the live statistics of the running round, if they are to be kept when the connection breaks
	const CStoredMatch *LiveStatsToPersist() const;
	...
	const std::optional<CStoredMatch> &LiveStats() const { return m_LiveStats; }
};
