...
{
	const auto SourceRank = [](EMatchReportSource Source) {
		switch(Source)
		{
		case EMatchReportSource::CLIENT_OBSERVED: return 0;
		case EMatchReportSource::SERVER_SNAPSHOT: return 1;
		case EMatchReportSource::SERVER_REPORT: return 2;
		case EMatchReportSource::SERVER_DATABASE: return 3;
		}
		return 0;
	};
	const auto CompletenessRank = [](EMatchCompleteness Completeness) {
		switch(Completeness)
		{
		case EMatchCompleteness::ABORTED: return 0;
		case EMatchCompleteness::PARTIAL_SINCE_JOIN: return 1;
		case EMatchCompleteness::COMPLETE: return 2;
		}
		return 0;
	};
	if(SourceRank(NewSource) != SourceRank(StoredSource))
		return SourceRank(NewSource) > SourceRank(StoredSource);
	return CompletenessRank(NewCompleteness) > CompletenessRank(StoredCompleteness);
}
...
CMatchJournal::EInsertResult CMatchJournal::InsertImpl(const CStoredMatch &Match, const char *pObservedOriginId, CUuid ObservedMatchId, std::string *pError)
{
	...
	// A round can be stored twice: the client keeps the live snapshot it had
	// when the connection broke, and the server sends the finished report of the
	// same round afterwards, under the same match id. The insert below ignores
	// the second one, so the better version has to displace the worse one here.
	// Its rows in the other tables go with it, through the foreign keys.
	CSqliteStmt pSelectExisting;
	if(!Prepare(pSqlite, "SELECT source, completeness FROM matches WHERE origin_id = ? AND match_id = ?", pSelectExisting, pError) ||
		!SqlSuccess(sqlite3_bind_int64(pSelectExisting.get(), 1, OriginId), pSqlite, pError, "bind existing origin") ||
		!BindText(pSqlite, pSelectExisting.get(), 2, MatchId, pError))
		return FailInsert();
	const int ExistingStep = sqlite3_step(pSelectExisting.get());
	if(ExistingStep != SQLITE_ROW && ExistingStep != SQLITE_DONE)
	{
		SetSqlError(pSqlite, pError, "select stored match");
		return FailInsert();
	}
	if(ExistingStep == SQLITE_ROW)
	{
		const int ExistingSource = sqlite3_column_int(pSelectExisting.get(), 0);
		const int ExistingCompleteness = sqlite3_column_int(pSelectExisting.get(), 1);
		pSelectExisting.reset();
		if(ValidSource(ExistingSource) && ValidCompleteness(ExistingCompleteness) &&
			MatchSupersedes(Match.m_Source, Match.m_Completeness, static_cast<EMatchReportSource>(ExistingSource), static_cast<EMatchCompleteness>(ExistingCompleteness)))
		{
			CSqliteStmt pDeleteSuperseded;
			if(!Prepare(pSqlite, "DELETE FROM matches WHERE origin_id = ? AND match_id = ?", pDeleteSuperseded, pError) ||
				!SqlSuccess(sqlite3_bind_int64(pDeleteSuperseded.get(), 1, OriginId), pSqlite, pError, "bind superseded origin") ||
				!BindText(pSqlite, pDeleteSuperseded.get(), 2, MatchId, pError) ||
				!StepDone(pSqlite, pDeleteSuperseded.get(), pError))
				return FailInsert();
		}
	}
	pSelectExisting.reset();
