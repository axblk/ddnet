...
/**
 * Decides whether a report of a round takes the place of one the journal
 * already holds for the same server and the same match id.
 *
 * A round can arrive twice. The client stores the live snapshot it was holding
 * when the connection broke, and after a reconnect the server sends the
 * finished report of that same round. Both carry the same match id, so one of
 * them has to give way, and it is the one that knows less: first by who said it
 * - the server's own report over a snapshot over what the client watched - and
 * within that by how much of the round it covers.
 */
