...
/**
 * Hits over shots, as a percentage.
 *
 * Capped at a hundred, because the two are not counted the same way: a shot is
 * one call of the weapon, a hit is one damage event, and a shotgun blast whose
 * three bullets land or a grenade that catches two players produce several of
 * the second for one of the first. The ratio is still the right number to look
 * at, but reading "300%" next to a column called accuracy says the counter is
 * broken when it is not. The shots and the hits are shown in their own columns
 * for anyone who wants to see where the surplus comes from.
 */
inline void FormatMatchAccuracy(int64_t Hits, int64_t Shots, char *pBuffer, int BufferSize)
{
	if(Shots <= 0)
		...
		str_format(pBuffer, BufferSize, "%.1f%%", std::min(100.0, 100.0 * static_cast<double>(Hits) / static_cast<double>(Shots)));
}
