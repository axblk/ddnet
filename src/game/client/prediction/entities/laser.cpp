...
bool CLaser::HitCharacter(vec2 From, vec2 To)
{
	...
	CCharacter *pHit;
	bool DontHitSelf = (GameWorld()->GameConfig()->m_SvOldLaser || !GameWorld()->m_WorldConfig.m_IsDDRace) || (m_Bounces == 0);
	...
	if(!pHit || (pHit == pOwnerChar && GameWorld()->GameConfig()->m_SvOldLaser) || (pHit != pOwnerChar && pOwnerChar ? (pOwnerChar->LaserHitDisabled() && m_Type == WEAPON_LASER) || (pOwnerChar->ShotgunHitDisabled() && m_Type == WEAPON_SHOTGUN) : !GameWorld()->GameConfig()->m_SvHit))
		return false;
	m_From = From;
	m_Pos = At;
	m_Energy = -1;
	if(m_Type == WEAPON_SHOTGUN)
	{
		...
		if(!GameWorld()->GameConfig()->m_SvOldLaser)
		{
			if(m_PrevPos != HitPos)
			{
		...
		else if(GameWorld()->GameConfig()->m_SvOldLaser && pOwnerChar)
		{
