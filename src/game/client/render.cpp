...
void CRenderTools::RenderTee7(const CAnimState *pAnim, const CTeeRenderInfo *pInfo, int Emote, vec2 Dir, vec2 Pos, float Alpha) const
{
	vec2 Direction = Dir;
	vec2 Position = Pos;
	const bool IsBot = pInfo->m_Sixup.m_BotTexture.IsValid();
		...
		for(int Filling = 0; Filling < 2; Filling++)
		{
			...
			if(Filling == 1)
			{
				...
				if(IsBot && !OutLine)
				{
					...
					Item = BotItem;
					Graphics()->QuadsDraw(&Item, 1);
					Graphics()->SetColor(pInfo->m_Sixup.m_BotColor.WithAlpha(Alpha));
				...
				// draw decoration
				const IGraphics::CTextureHandle &DecorationTexture = pInfo->m_Sixup.PartTexture(protocol7::SKINPART_DECORATION);
				if(DecorationTexture.IsValid())
				{
					Graphics()->TextureSet(DecorationTexture);
					Graphics()->QuadsBegin();
					Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle * pi * 2);
					Graphics()->SetColor(pInfo->m_Sixup.m_aColors[protocol7::SKINPART_DECORATION].WithAlpha(Alpha));
				...
				const IGraphics::CTextureHandle &BodyTexture = pInfo->m_Sixup.PartTexture(protocol7::SKINPART_BODY);
				...
				{
					Graphics()->SetColor(pInfo->m_Sixup.m_aColors[protocol7::SKINPART_BODY].WithAlpha(Alpha));
				...
				// draw marking
				const IGraphics::CTextureHandle &MarkingTexture = pInfo->m_Sixup.PartTexture(protocol7::SKINPART_MARKING);
				...
				// draw eyes
				Graphics()->TextureSet(pInfo->m_Sixup.PartTexture(protocol7::SKINPART_EYES));
				Graphics()->QuadsBegin();
				Graphics()->QuadsSetRotation(pAnim->GetBody()->m_Angle * pi * 2);
				if(IsBot)
				{
				...
				{
					Graphics()->SetColor(pInfo->m_Sixup.m_aColors[protocol7::SKINPART_EYES].WithAlpha(Alpha));
				}
				if(Pass == 1)
				{
					switch(Emote)
			...
			// draw feet
			Graphics()->TextureSet(pInfo->m_Sixup.PartTexture(protocol7::SKINPART_FEET));
			Graphics()->QuadsBegin();
