...
public:
	CGameSessionContext(CSessionId Id, const char *pMapName, EGameProtocol Protocol, const std::vector<CStreamId> &vStreamIds) :
		m_Id(Id),
		m_MapName(pMapName),
		m_Protocol(Protocol)
	{
...
public:
	CGameSessionContext *Create(CSessionId Id, const char *pMapName, EGameProtocol Protocol, const std::vector<CStreamId> &vStreamIds)
	{
		if(!Id.IsValid() || Find(Id))
			return nullptr;
		m_vpContexts.push_back(std::make_unique<CGameSessionContext>(Id, pMapName, Protocol, vStreamIds));
