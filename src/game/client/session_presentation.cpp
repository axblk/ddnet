...
CStateClientPresentation::CStateClientPresentation()
{
...
void CSessionPresentation::UpdateClients(const CPresentationContext &Context)
{
	...
	if(Context.m_Session.Protocol() == EGameProtocol::SIXUP)
	{
		...
		{
			const bool SessionLocal = std::any_of(Context.m_Session.GameStates().begin(), Context.m_Session.GameStates().end(), [ClientId](const CGameState &OtherState) { return OtherState.LocalClientId() == ClientId; });
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ++ClientId)
	{
		...
		for(const CGameState &SessionState : Context.m_Session.GameStates())
			SessionLocal |= SessionState.LocalClientId() == ClientId;
		...
		if(!Context.m_Time.m_IsDemoPlayback)
		{
			...
			else
			{
				for(const CGameState &SessionState : Context.m_Session.GameStates())
				{
					if(SessionState.LocalClientId() == ClientId)
					{
						pInputState = &SessionState;
						break;
					}
				}
			}
		...
		bool Frozen = false;
		// Only the state that takes input is predicted, the local player of any other one is read off the snapshot.
	...
	const bool ReceivedFinishTimes = State.m_Runtime.m_ReceivedDDNetPlayerFinishTimes;
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ++ClientId)
	{
		const bool ActiveInSession = std::any_of(Context.m_Session.GameStates().begin(), Context.m_Session.GameStates().end(), [ClientId](const CGameState &OtherState) { return OtherState.Client(ClientId).m_HasPlayerInfo; });
		if(!ActiveInSession)
...
void CSessionPresentation::UpdateMapSounds(const CGameState &State, const CGameTickInfo &Time, const CGameView &View, bool UsePredictedTime, bool Offline)
{
	...
	m_MapSounds.Update(State, Time, View.CameraPosition(), Time.m_IsDemoPlaybackPaused, View.RenderOptions().m_HighDetail, m_pMap->m_LayersBackground.EnvEvaluator(), Offline);
}
...
{
	for(auto &pPresentation : m_vpPresentations)
		pPresentation->MapSounds().StopVoices();
}
