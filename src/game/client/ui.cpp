...
void CUi::ClipEnable(const CUIRect *pRect)
{
	if(IsClipped())
	{
		...
		Intersection.w = std::max(0.0f, std::min(pRect->x + pRect->w, pOldRect->x + pOldRect->w) - Intersection.x);
		Intersection.h = std::max(0.0f, std::min(pRect->y + pRect->h, pOldRect->y + pOldRect->h) - Intersection.y);
...
void CUi::DrawButton_FontIcon(const char *pText, const CUIRect *pRect, ColorRGBA Color, int Corners, bool Enabled, const void *pCacheId)
{
	...
	if(pCacheId != nullptr)
	{
		auto [Iterator, Inserted] = m_FontIconUiElements.try_emplace(pCacheId, nullptr);
		if(Inserted)
			Iterator->second = GetNewUIElement(2);
		pUiElement = Iterator->second;
	}
	if(pUiElement != nullptr)
		DoLabelStreamed(*pUiElement->Rect(0), &Label, pText, Label.h * ms_FontmodHeight, TEXTALIGN_MC);
	else
	...
	if(!Enabled)
	{
		...
		if(pUiElement != nullptr)
			DoLabelStreamed(*pUiElement->Rect(1), &Label, FontIcon::SLASH, Label.h * ms_FontmodHeight, TEXTALIGN_MC);
		else
...
{
	DrawButton_FontIcon(pText, pRect, ButtonColor.value_or(ColorRGBA(1.0f, 1.0f, 1.0f, (Checked ? 0.1f : 0.5f) * ButtonColorMul(pButtonContainer))), Corners, Enabled, pButtonContainer);
