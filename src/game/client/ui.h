...
public:
	...
	float PixelSize();
	/**
	 * Lays the UI out in a part of the screen, given in pixels, instead of all
	 * of it: one view of several that share the window. Empty for all of it
	 * again.
	 */
	...
	void SetRenderPopupMenuBackdropCallback(std::function<void(const CUIRect &, int, float)> pfnCallback) { m_RenderPopupMenuBackdropFunction = std::move(pfnCallback); }
