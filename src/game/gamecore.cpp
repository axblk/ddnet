...
void CCharacterCore::Tick(bool UseInput, bool DoDeferredTick)
{
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		int TeleNr = 0;
		int Hit = DDNetPhysics ? m_pCollision->IntersectLineTeleHook(m_HookPos, NewPos, &NewPos, nullptr, &TeleNr, m_pWorld->m_OldTeleportHook) : m_pCollision->IntersectLine(m_HookPos, NewPos, &NewPos, nullptr);
