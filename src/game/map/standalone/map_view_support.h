		...
		void LoadMapImages()
		{
			...
			for(int i = 0; i < m_Count; i++)
			{
				...
				if(pImg->m_External)
				{
					...
					vLoads.push_back({i, LoadFlags, m_pAssetLoader->LoadImageFile(m_pStorage, aPath, IStorage::TYPE_ALL)});
				}
				...
				else
				{
					...
					else
					{
						...
						vLoads.push_back({i, LoadFlags, m_pAssetLoader->LoadImageRawData(std::move(RawData), pImg->m_Width, pImg->m_Height, CImageInfo::FORMAT_RGBA, aTexName)});
					}
				}
				if(pImg->m_ImageName >= 0)
					m_pMap->UnloadData(pImg->m_ImageName);
