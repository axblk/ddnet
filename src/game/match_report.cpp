...
bool CMatchReportBuilder::Finalize(std::string *pError)
{
	...
	// One rejected addition - a limit reached, a metric a mode never declared, a
	// counter that would have overflowed - costs that one number and not the
	// round. Everything the report does carry is still true, so it is handed on
	// and marked as not countable, the same graded answer the participant limit
	// already gives.
	if(m_NumRejectedMutations > 0 && m_Report.m_Ranked)
	{
		m_Report.m_Ranked = false;
		m_Report.m_UnrankedReason = "incomplete";
	}
