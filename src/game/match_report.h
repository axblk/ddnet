...
public:
	...
	/**
	 * Counts the additions the builder turned down: a limit that was reached, a
	 * duplicate, a counter that would have overflowed.
	 *
	 * A caller that wants to know whether it lost anything asks after finalizing;
	 * the report itself says so by being unranked.
	 */
	int NumRejectedMutations() const { return m_NumRejectedMutations; }
