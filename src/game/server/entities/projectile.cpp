...
void CProjectile::Tick()
{
	...
	if(GameServer()->m_World.m_Core.m_PhysicsRuleset == EPhysicsRuleset::DDNET)
	{
		const int Index = GameServer()->Collision()->GetIndex(PrevPos, CurPos);
		const int TeleNumber = GameServer()->m_World.m_Core.m_OldTeleportWeapons ? GameServer()->Collision()->IsTeleport(Index) : GameServer()->Collision()->IsTeleportWeapon(Index);
