...
{
	m_GameHost.Controller()->OnPlayerPracticeTeleport(ClientId, vec2((float)pMsg->m_X, (float)pMsg->m_Y));
}
	...
	for(int y = 0; y < m_Collision.GetHeight(); y++)
	{
		for(int x = 0; x < m_Collision.GetWidth(); x++)
		{
			...
			// Game layer
			{
				...
				{
					m_GameHost.Controller()->OnEntity({GameIndex - ENTITY_OFFSET, x, y, LAYER_GAME, pTiles[Index].m_Flags, Initial, 0});
				}
			}
			...
			if(pFront)
			{
				...
				{
					m_GameHost.Controller()->OnEntity({FrontIndex - ENTITY_OFFSET, x, y, LAYER_FRONT, pFront[Index].m_Flags, Initial, 0});
				}
			}
