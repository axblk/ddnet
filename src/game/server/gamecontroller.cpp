...
CTuningParams IGameController::DefaultTuning() const
{
	return CTuningParams::DEFAULT;
}

void IGameController::ResetTuningZones()
{
...
bool IGameController::OnEntity(const CMapEntityContext &Context)
{
	...
	if(Index >= ENTITY_SPAWN && Index <= ENTITY_SPAWN_BLUE)
	{
		if(Context.m_Initial)
			m_avSpawnPoints[Index - ENTITY_SPAWN].emplace_back(Context.m_X * 32.0f + 16.0f, Context.m_Y * 32.0f + 16.0f);
		return true;
	}
	...
	return CreatePickup(Type, SubType, Context);
}

bool IGameController::CreatePickup(int Type, int SubType, const CMapEntityContext &Context)
{
	CPickup *pPickup = new CPickup(&GameServer()->m_World, Type, SubType, Context.m_Layer, Context.m_Number, TileFlagsToPickupFlags(Context.m_Flags));
	pPickup->m_Pos = vec2(Context.m_X * 32.0f + 16.0f, Context.m_Y * 32.0f + 16.0f);
...
{
	return {true, false, 0.001f, Context.m_OwnerConnected ? EProjectileOwnerLossAction::KEEP : EProjectileOwnerLossAction::DETACH, false};
}
