...
bool IGameController::BuildLiveStatsReport(int ClientId, CMatchReport &Report, int &LocalParticipantId, std::string &Payload)
{
	...
	if(!pPlayer)
		return false;
	// The snapshot is the running match as it stands, so everyone's numbers are
	// brought up to this tick, not only those of the player who asked. What the
	// client draws from it is the same table the final report produces, and a
	// table of one row would say less than the scoreboard next to it.
	for(CPlayer *pOther : GameServer()->m_apPlayers)
		if(pOther)
			UpdateMatchParticipant(pOther, pOther->GetTeam() == TEAM_SPECTATORS);
	CMatchParticipantState *pState = MatchParticipant(pPlayer);
	if(!pState)
		return false;
	...
	Report.m_Termination = EMatchTermination::ABORTED;
	const int64_t DurationTicks = Report.m_DurationTicks;
	...
	{
		CMatchParticipant Participant = State.m_Participant;
		// Still playing means not gone yet: a leaving tick belongs to the final
		// report, where the match is over for everyone.
		if(State.m_ActiveSinceTick >= 0)
			Participant.m_LeftTick.reset();
		Participant.m_JoinedTick = std::clamp<int64_t>(Participant.m_JoinedTick, 0, DurationTicks);
		const int ParticipantId = State.m_Participant.m_ParticipantId;
		...
		if(!Builder.AddParticipant(std::move(Participant)) ||
			!Builder.SetMetricValue(EMatchSubjectKind::PARTICIPANT, ParticipantId, ScoreMetricId.c_str(), State.m_Score, MatchMetricAggregation(ScoreMetricId.c_str())) ||
			!Builder.SetMetricValue(EMatchSubjectKind::PARTICIPANT, ParticipantId, PlaytimeMetricId.c_str(), PlaytimeTicks, MatchMetricAggregation(PlaytimeMetricId.c_str())))
			return false;
	}
...
void IGameController::FinalizeMatchReport(EMatchTermination Termination)
{
	...
	std::string Error;
	std::string Payload;
	if(m_pMatchReportBuilder->NumRejectedMutations() > 0)
		log_warn("game", "match report is incomplete, %d additions were turned down", m_pMatchReportBuilder->NumRejectedMutations());
