...
static const CGameModeRegistration gs_CTF({"ctf", "CTF", protocol7::GAMEFLAG_TEAMS | protocol7::GAMEFLAG_FLAGS, false}, NewGameController<CGameControllerVanillaCTF>);
...
bool CGameControllerVanillaCTF::OnEntity(const CMapEntityContext &Context)
{
	if(Context.m_Index != ENTITY_FLAGSTAND_RED && Context.m_Index != ENTITY_FLAGSTAND_BLUE)
		return CGameControllerVanillaTeamplay::OnEntity(Context);
...
void CGameControllerVanillaCTF::Tick()
{
	...
	// in sudden death only a capture decides, not a grab
	const bool Tied = Match().IsSuddenDeath() ? m_aTeamScores[TEAM_RED] / 100 == m_aTeamScores[TEAM_BLUE] / 100 : m_aTeamScores[TEAM_RED] == m_aTeamScores[TEAM_BLUE];
	CheckMatchEnd(std::max(m_aTeamScores[TEAM_RED], m_aTeamScores[TEAM_BLUE]), Tied);
}
...
void CGameControllerVanillaCTF::FlagGrab(CFlag *pFlag, CCharacter *pCarrier)
{
	...
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
		...
		if(!pPlayer)
			continue;
		// a spectator hears it the way the player they follow does
		const CPlayer *pListener = pPlayer->GetTeam() == TEAM_SPECTATORS && Services().Player(pPlayer->SpectatorId()) ? Services().Player(pPlayer->SpectatorId()) : pPlayer;
		Services().CreateLegacySoundGlobal(pListener->GetTeam() == pFlag->Team() ? SOUND_CTF_GRAB_EN : SOUND_CTF_GRAB_PL, ClientId);
	}
...
void CGameControllerVanillaCTF::FlagCapture(CFlag *pFlag)
{
	...
	// 0.7 clients print this themselves from the game message below
	char aBuf[128];
	const char *pFlagColor = pFlag->Team() == TEAM_RED ? "red" : "blue";
	if(CaptureTime <= 60.0f)
		str_format(aBuf, sizeof(aBuf), "The %s flag was captured by '%s' (%.2f seconds)", pFlagColor, Server()->ClientName(CarrierId), CaptureTime);
	else
		str_format(aBuf, sizeof(aBuf), "The %s flag was captured by '%s'", pFlagColor, Server()->ClientName(CarrierId));
	Services().SendLegacyChatGlobal(aBuf);
	for(int ClientId = 0; ClientId < MAX_CLIENTS; ClientId++)
	{
