...
/// The host of a server URL, the IP address it is if it is one, and the
/// fragment. Anything else in the URL is rejected.
fn host_and_fragment(
    url: &Url,
    protocol: Protocol,
) -> Option<(&str, Option<IpAddr>, Option<Fragment>)> {
    if url.path() != ""
        || url.query().is_some()
    ...
    let fragment = match url.fragment() {
        Some(fragment) if valid_fragment(protocol, fragment) => {
            Some(Fragment::from(fragment).ok()?)
        }
        Some(_) => return None,
        None => None,
    };
    Some((host, ip, fragment))
}
...
impl FromStr for Addr {
    ...
    fn from_str(s: &str) -> Result<Addr, InvalidAddr> {
        ...
        let (host, ip, fragment) = host_and_fragment(&url, protocol).ok_or(InvalidAddr)?;
        if port == 0 {
            return Err(InvalidAddr);
        }
        let host = match ip {
            Some(ip) => Host::Ip(ip),
            None => Host::Name(Hostname::from(host).map_err(|_| InvalidAddr)?),
        };
        Ok(Addr {
            host,
            port,
...
impl FromStr for RegisterAddr {
    ...
    fn from_str(s: &str) -> Result<RegisterAddr, ParseRegisterAddrError> {
        ...
        let (host, ip, fragment) = host_and_fragment(&url, protocol).ok_or(Error::InvalidHost)?;
        let host = if host == "connecting-address.invalid" {
            None
        } else if ip.is_some() {
            return Err(Error::InvalidHost);
        } else {
            Some(Hostname::from(host).map_err(|_| Error::InvalidHost)?)
        };
...
mod test {
    ...
    fn register_addr_from_str() {
        ...
        assert_eq!(
            RegisterAddr::from_str(address).unwrap().to_string(),
            address
        );
        ...
        assert!(
            RegisterAddr::from_str("tw-0.6+udp://connecting-address.invalid:8303#webpki").is_err()
        );
    }
}
