...
///   wire, for the transports that do (`ddnet+quic`, `tw-0.7+quic`,
///   `ddnet+ws`, `ddnet+wss`).
/// - `cert-sha256=<hash>[,<next>]`: the TLS/QUIC certificate to pin by hash,
///   for the transports a browser reaches (`ddnet+wt`, `tw-0.7+wt`,
///   `ddnet+wss`) as well as native QUIC; a second hash covers a rotation.
/// - `webpki`: a certificate from a public CA, for the TLS-cert transports
///   (`ddnet+wt`, `tw-0.7+wt`, `ddnet+wss`).
fn valid_fragment(protocol: Protocol, fragment: &str) -> bool {
    use self::Protocol::*;
    let identity = matches!(protocol, Quic | Quic7 | Ws | Wss);
    let certificate = matches!(protocol, Quic | Quic7 | WebTransport | WebTransport7 | Wss);
    let webpki = matches!(protocol, WebTransport | WebTransport7 | Wss);
    if fragment == "webpki" {
        return webpki;
    }
...
mod test {
    ...
    fn fragments() {
        ...
        assert!(Addr::from_str(&format!("ddnet+wss://game.example.org:8303{two}")).is_ok());
        ...
        assert!(Addr::from_str(&format!("ddnet+wss://game.example.org:8303{same}")).is_err());
