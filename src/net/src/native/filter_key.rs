...
//!
//! An XDP filter decides without keeping state whether a packet belongs to
//! someone the server knows: it recomputes the security token from the
//! source address, or the tag in a QUIC connection ID, with SipHash-2-4
//! under a key it owns. The server derives both the same way from the same
//! key. The filter rotates the key and keeps two epochs valid, and so does
//! the server. Without a key the tokens come from `Challenger`'s own
//! secrets.
...
/// SipHash-2-4, as the filter computes it.
...
mod test {
    ...
    // The expected tokens under this key were produced with the filter's C
    // implementation.
