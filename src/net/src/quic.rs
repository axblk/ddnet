...
pub struct Connection {
    ...
    /// The messages of the datagrams received, in order.
...
impl Connection {
    ...
    fn next_datagram_message(&mut self, buf: &mut [u8]) -> Result<Option<usize>> {
        loop {
            if let Some(message) = self.incoming_messages.pop_front() {
                buf[..message.len()].copy_from_slice(&message);
                return Ok(Some(message.len()));
            }
            let Some(dgram) = self
                .inner
                .dgram_recv_vec()
            ...
            while let Some(message) = datagram.next_message() {
                self.incoming_messages.push_back(message.to_vec());
            }
        }
    }
