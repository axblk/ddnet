//! The framing DDNet speaks on top of a stream-and-datagram transport.
//!
...
fn frame_limit(frame_type: u64) -> Option<usize> {
    match frame_type {
...
pub fn decode_datagram(data: &[u8]) -> Result<Datagram<'_>, DecodeError> {
    ...
    let _sequence = read_varint(data, &mut offset)?;
...
mod tests {
    ...
    fn golden_vectors() {
        ...
        assert_eq!(decode_frame(&[0x40, 0x40, 0x00]).unwrap().frame_type, SKIPPABLE_FRAME_START);
