...
TEST(ClientConnection, StreamValueStorageGrowsWithoutChangingExistingValues)
{
	...
	// A stream that appears later has to start out like the ones that were there
	// from the beginning. For a client id that is -1; growing to 0 would name
	// the first player on the server.
	CStreamStorage<int> ClientIds(2, -1);
	EXPECT_EQ(ClientIds[1], -1);
	EXPECT_EQ(ClientIds[2], -1);
	EXPECT_EQ(ClientIds[7], -1);
	Values.Fill(-1);
