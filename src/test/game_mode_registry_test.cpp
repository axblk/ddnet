...
TEST(GameModeRegistry, BuiltInMetadata)
{
	...
	// Eleven metrics that stand on their own and six weapons with seven each.
	ASSERT_EQ(pVanillaDM->m_Report.m_vMetrics.size(), 53u);
	...
	EXPECT_EQ(pVanillaDM->m_Report.m_vMetrics[11].m_Id, "vanilla.dm@ddnet.org/weapon_0_kills");
	EXPECT_EQ(pVanillaDM->m_Report.m_vMetrics[52].m_Id, "vanilla.dm@ddnet.org/weapon_5_damage_taken");
	...
	ASSERT_EQ(pVanillaCTF->m_Report.m_vMetrics.size(), 56u);
	...
	ASSERT_EQ(pZCatch->m_Report.m_vMetrics.size(), 54u);
