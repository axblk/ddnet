...
namespace
{
	bool ApplySnapshot(CGameState &State, int Tick, const CSnapshot *pSnapshot, int Size)
	{
		...
		State.ApplySnapshotData(Tick, pSnapshot->NumItems(), aClients);
		return true;
	}
...
TEST(GameState, TeamsAreIndependentAndResetPerState)
{
	...
	Primary.ApplySnapshotData(10, 1, aPrimaryClients);
	...
	Additional.ApplySnapshotData(20, 1, aAdditionalClients);
...
TEST(GameState, RenderedClientsUseStateOwnedSnapshotHistory)
{
	...
	Primary.ApplySnapshotData(10, 4, *pPrimaryClients);
	Additional.ApplySnapshotData(20, 4, *pAdditionalClients);
	...
	Primary.ApplySnapshotData(30, 2, *pIncompleteClients);
...
TEST(GameState, ClientIdentityAndEmoticonsAreIndependentAndResetPerState)
{
	...
	Primary.ApplySnapshotData(10, 2, *pPrimaryClients);
	Additional.ApplySnapshotData(20, 2, *pAdditionalClients);
	...
	Primary.ApplySnapshotData(12, 1, *pPrimaryWithoutClientInfo);
	...
	Primary.ApplySnapshotData(13, 0, *pEmptyClients);
...
TEST(GameState, Protocol7ClientsAreOwnedAndResetPerState)
{
	...
	aClients[5] = {};
	Primary.ApplySnapshotData(2, 0, aClients);
...
TEST(GameState, ExtendedPlayersAndSpectatorCharactersAreIndependentAndResetPerState)
{
	...
	pPrimary->ApplySnapshotData(10, 2, aPrimaryClients);
	pAdditional->ApplySnapshotData(10, 2, aAdditionalClients);
	...
	pPrimary->ApplySnapshotData(30, 0, aEmptyClients);
...
TEST(GameState, EnvelopeTimeUsesFrozenStateTicks)
{
	...
	std::array<CGameState::CClientSnapshot, MAX_CLIENTS> aClients = {};
	State.ApplySnapshotData(101, 1, aClients, &GameInfo);
	...
	aClients = {};
	State.ApplySnapshotData(201, 1, aClients, &GameInfo);
...
TEST(GameState, SessionsOwnDifferentMapsProtocolsAndStates)
{
	...
	EXPECT_EQ(pNetwork->MapContext().GameConfig().Values().m_SvHit, 0);
	EXPECT_EQ(pDemo->MapContext().GameConfig().Values().m_SvHit, 1);
...
TEST(GameState, NetworkSourcesDriveThreeStreamsAndTwoSessions)
{
	...
	ASSERT_TRUE(pSecondSourceRaw->SetState(ESessionState::LOADING_MAP));
...
TEST(GameState, SessionInfoMessagesAreBoundedAndIndependent)
{
	...
	for(int Tick = 10; Tick <= 60; Tick += 10)
	{
		...
		Network.InfoMessages().Add(Message);
	}
	...
	Demo.InfoMessages().Add(DemoMessage);
...
TEST(GameState, SessionChatIsBoundedAndIndependent)
{
	...
	const CSessionChatState::CLine &StoredRepeated = Network.Chat().Add(Repeated);
...
TEST(GameState, SessionStatsFollowTheirSnapshotLifecycle)
{
	...
	State.ApplySnapshotData(200, 2, aClients, &GameInfo);
...
TEST(GameState, SnapshotWorldsAreIndependent)
{
	...
	Primary.ApplySnapshotData(100, 2, aPrimaryClients);
	Additional.ApplySnapshotData(200, 2, aAdditionalClients);
...
TEST(GameState, EntitySnapshotsOwnCurrentAndPreviousData)
{
	...
	Additional.ApplySnapshotData(20, 0, aClients);
...
TEST(GameView, RenderingTwoViewsDoesNotAdvanceState)
{
	...
	State.SetTeam(4, 1);
	State.SetTeam(5, 2);
	State.ApplySnapshotData(50, 3, aClients);
	...
	pLeft->MultiView().m_Active = true;
	pLeft->MultiView().m_Team = 7;
	pLeft->MultiView().m_aSelected[5] = true;
	pRight->MultiView().m_Team = 9;
	...
	EXPECT_EQ(pLeft->Viewport(), (CViewport{0, 0, 640, 720}));
	EXPECT_EQ(pLeft->CameraPosition(), vec2(100.0f, 200.0f));
	EXPECT_FALSE(pLeft->IsSpectating());
	EXPECT_TRUE(pLeft->MultiView().m_Active);
	EXPECT_EQ(pLeft->MultiView().m_Team, 7);
	EXPECT_TRUE(pLeft->MultiView().m_aSelected[5]);
	EXPECT_EQ(pRight->Viewport(), (CViewport{640, 0, 640, 720}));
	EXPECT_EQ(pRight->CameraPosition(), vec2(300.0f, 400.0f));
	EXPECT_TRUE(pRight->IsSpectating());
	EXPECT_EQ(pRight->SpectatorId(), SPEC_FREEVIEW);
	EXPECT_EQ(pRight->MultiView().m_Team, 9);
...
TEST(GameView, PresentationContextCombinesVisibleWorldRectsWithoutView)
{
	...
	pState->SetTeam(1, 1);
	pState->SetTeam(2, 2);
	pState->ApplySnapshotData(50, 1, aClients);
...
TEST(GameView, MultiViewStatesAreIndependentAndReset)
{
	...
	pLeft->SetTarget(CSessionId(1), CGameStateId(2));
	EXPECT_EQ(Left.m_Team, 3);
	EXPECT_TRUE(Left.m_Active);
	EXPECT_TRUE(Left.m_aSelected[7]);
	pLeft->SetTarget(CSessionId(2), CGameStateId(2));
...
TEST(GameState, HeadlessSpectatorSnapshotHasNoDesktopDependencies)
{
	...
	aClients[5].m_PlayerInfo.m_Team = TEAM_SPECTATORS;
	State.ApplySnapshotData(80, 2, aClients);
...
TEST(GameState, GeneratedDemoPlaysToKnownDigestHeadlessly)
{
	...
	Expected.ApplySnapshotData(300, 2, aExpectedClients);
