...
TEST(GraphicsThreaded, TextureDescriptorsValidateRenderTargetUsage)
{
	...
	Desc.m_Usage = IGraphics::TEXTURE_USAGE_SAMPLED;
	EXPECT_TRUE(Desc.IsValid());
	Desc.m_Mipmaps = IGraphics::ETextureMipmaps::GENERATE;
	EXPECT_FALSE(Desc.IsValid());
	Desc.m_Mipmaps = IGraphics::ETextureMipmaps::NONE;
...
TEST(GraphicsThreaded, CommandBufferStorageTransfersWithoutCopyingPayload)
{
	...
	EXPECT_TRUE(BackendBuffer.SubmissionInfo().m_EndsFrame);
...
TEST(GraphicsThreaded, TextureUpdatesUseTypedUploadSizes)
{
	...
	EXPECT_EQ(pStoredUpdate->m_Format, IGraphics::ETextureFormat::RG8_UNORM);
...
namespace
{
	// Tags the buffer with a clear command carrying the id, so the tests can
	// tell buffers apart after the queue swapped them into its own.
	void SetSubmission(CCommandBuffer &Buffer, CCommandBuffer::ECommandChannel Channel, int Id, bool EndsFrame = false)
	{
		CCommandBuffer::SCommand_Clear Clear;
		Clear.m_Color = ColorRGBA(Id, 0.0f, 0.0f, 1.0f);
		EXPECT_TRUE(Buffer.AddCommandUnsafe(Clear));
		CCommandBuffer::SSubmissionInfo Submission;
	...
	int BufferId(const CRenderCommandQueue::SEntry &Entry)
	{
		return (int)static_cast<const CCommandBuffer::SCommand_Clear *>(Entry.m_pBuffer->Head())->m_Color.r;
	}
}
...
TEST(GraphicsThreaded, RenderCommandQueueWaitsOnlyForReliableCapacity)
{
	...
	ASSERT_TRUE(Queue.WaitEnqueueReliable(&Reliable1));
	ASSERT_TRUE(Queue.WaitEnqueueReliable(&Reliable2));
	ASSERT_TRUE(Queue.WaitEnqueueReliable(&Reliable3));
