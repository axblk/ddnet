...
TEST_F(MatchJournal, StoresDisconnectSnapshotWithoutDowngradingFinalReport)
{
	...
	// Nothing is deleted by hand here: the round is already in the journal as a
	// snapshot, and the server's finished report of the same round takes its
	// place, which is what the production path relies on after a reconnect.
	...
	// The other direction does not happen: a snapshot arriving after the report
	// leaves it alone.
	EXPECT_EQ(m_Journal.Insert(Snapshot, &Error), CMatchJournal::EInsertResult::DUPLICATE) << Error;
	ASSERT_TRUE(m_Journal.LoadMatch(Final.m_OriginId.c_str(), Final.m_Report.m_MatchId, Loaded, &Error)) << Error;
	EXPECT_EQ(Loaded.m_Source, EMatchReportSource::SERVER_REPORT);
	std::vector<CMatchHistoryEntry> vStored;
	ASSERT_TRUE(m_Journal.ListMatches({}, vStored, &Error)) << Error;
	EXPECT_EQ(vStored.size(), 1u);
}
