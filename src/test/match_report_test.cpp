...
TEST(MatchReportBuilder, RejectsMetricOverflow)
{
	...
	std::string Error;
	// The round survives the rejected counter, it just stops being countable.
	EXPECT_TRUE(Builder.Finalize(&Error)) << Error;
	EXPECT_EQ(Builder.NumRejectedMutations(), 1);
	EXPECT_FALSE(Builder.Report().m_Ranked);
	EXPECT_EQ(Builder.Report().m_UnrankedReason, "incomplete");
}
...
TEST(MatchReportView, BuildsWeaponCombatStatsAndAccuracy)
{
	...
	// A shot is one call of the weapon, a hit is one damage event, so a shotgun
	// blast can land more hits than shots. The ratio is capped rather than shown
	// as an accuracy above a hundred percent.
	FormatMatchAccuracy(5, 2, aAccuracy, sizeof(aAccuracy));
	EXPECT_STREQ(aAccuracy, "100.0%");
