...
TEST(NetAddr, FromUrlStringValid)
{
	...
	EXPECT_EQ(net_addr_from_url(&Addr, "ws://127.0.0.1:8303", nullptr, 0), 0);
	EXPECT_EQ(Addr.type, NETTYPE_WEBSOCKET_IPV4);
...
TEST(NetAddr, FromUrlLookup)
{
	...
	EXPECT_EQ(Addr.type, NETTYPE_WEBSOCKET_IPV6 | NETTYPE_WEBSOCKET_TLS);
	EXPECT_EQ(net_addr_from_url_lookup(&Addr, "ddnet-20+wss://[::1]:8303", NETTYPE_WEBSOCKET_IPV4), -1);
	EXPECT_EQ(net_addr_from_url_lookup(&Addr, "ddnet-20+wss://127.0.0.1:8303", NETTYPE_WEBSOCKET_IPV6), -1);
	EXPECT_EQ(net_addr_from_url_lookup(&Addr, "ddnet-20+wss://localhost:invalid", NETTYPE_ALL), -1);
	EXPECT_EQ(net_addr_from_url_lookup(&Addr, "ddnet-20+wss://localhost:8303junk", NETTYPE_ALL), -1);
	EXPECT_EQ(net_addr_from_url_lookup(&Addr, "ddnet-20+wss://localhost:65536", NETTYPE_ALL), -1);
	EXPECT_EQ(net_host_lookup("ws://127.0.0.1:8303", &Addr, NETTYPE_IPV4), -1);
}
