...
TEST(RenderTrace, RecordsAndSaves)
{
	...
	Frame.m_Render.m_aGpuRenderZoneNanoseconds[0] = 150000;
	Frame.m_Render.m_GpuRenderZoneMask = 1;
	Trace.RecordFrame(Frame);
	const std::array<std::string, 1> aZoneNames = {"test_zone"};
	ASSERT_TRUE(Trace.Stop(aZoneNames));
	...
	EXPECT_NE(str_find(pJson, "\"test_zone\""), nullptr);
	EXPECT_NE(str_find(pJson, "\"gpu_zones_ns\""), nullptr);
	EXPECT_NE(str_find(pJson, "150000"), nullptr);
